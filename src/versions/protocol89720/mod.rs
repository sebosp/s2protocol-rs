...
//! Generated code from source: ../s2protocol/json/protocol89720.json
use crate::*;
...
impl SVarUint32 {
#[tracing::instrument(name="89720::SVarUint32::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
...
tracing::debug!("res: {:?}", res);
    Ok((tail, Self::MUint6(u8::try_from(res).unwrap())))
    },
 1 => {
tracing::debug!("Variant tagged '1' for MUint14");
let (tail, res) = tagged_vlq_int(tail)?;
tracing::debug!("res: {:?}", res);
    Ok((tail, Self::MUint14(u32::try_from(res).unwrap())))
    },
 2 => {
tracing::debug!("Variant tagged '2' for MUint22");
let (tail, res) = tagged_vlq_int(tail)?;
tracing::debug!("res: {:?}", res);
    Ok((tail, Self::MUint22(u32::try_from(res).unwrap())))
    },
 3 => {
tracing::debug!("Variant tagged '3' for MUint32");
let (tail, res) = tagged_vlq_int(tail)?;
tracing::debug!("res: {:?}", res);
    Ok((tail, Self::MUint32(u32::try_from(res).unwrap())))
    },
...
#[tracing::instrument(name="89720::byte_aligned::SVersion::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
...
}Ok((tail, Self {m_flags: m_flags.expect("Missing m_flags from struct"),
...
#[tracing::instrument(name="89720::byte_aligned::Smd5::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
}Ok((tail, Self {m_data_deprecated: m_data_deprecated.expect("Missing m_data_deprecated from struct"),
...
#[tracing::instrument(name="89720::byte_aligned::ReplaySHeader::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
let (new_tail, parsed_m_elapsed_game_loops) = Self::parse_m_elapsed_game_loops(tail)?;
tail = new_tail;
m_elapsed_game_loops = Some(parsed_m_elapsed_game_loops);
continue;
} else {
tracing::error!("Field m_elapsed_game_loops with tag 3 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 4 => {
...
let (new_tail, parsed_m_use_scaled_time) = Self::parse_m_use_scaled_time(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_data_build_num) = Self::parse_m_data_build_num(tail)?;
tail = new_tail;
...
panic!("Unhandled duplicate field.");
 ...
 7 => {
tracing::debug!("Field [{i}]: tagged '7' for field m_replay_compatibility_hash");
 if m_replay_compatibility_hash.is_none() {
let (new_tail, parsed_m_replay_compatibility_hash) = Self::parse_m_replay_compatibility_hash(tail)?;
tail = new_tail;
m_replay_compatibility_hash = Some(parsed_m_replay_compatibility_hash);
continue;
} else {
tracing::error!("Field m_replay_compatibility_hash with tag 7 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 8 => {
tracing::debug!("Field [{i}]: tagged '8' for field m_ngdp_root_key_is_dev_data");
 if m_ngdp_root_key_is_dev_data.is_none() {
let (new_tail, parsed_m_ngdp_root_key_is_dev_data) = Self::parse_m_ngdp_root_key_is_dev_data(tail)?;
tail = new_tail;
m_ngdp_root_key_is_dev_data = Some(parsed_m_ngdp_root_key_is_dev_data);
continue;
} else {
tracing::error!("Field m_ngdp_root_key_is_dev_data with tag 8 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
...
}Ok((tail, Self {m_signature: m_signature.expect("Missing m_signature from struct"),
m_version: m_version.expect("Missing m_version from struct"),
m_type: m_type.expect("Missing m_type from struct"),
m_elapsed_game_loops: m_elapsed_game_loops.expect("Missing m_elapsed_game_loops from struct"),
m_use_scaled_time: m_use_scaled_time.expect("Missing m_use_scaled_time from struct"),
...
m_replay_compatibility_hash: m_replay_compatibility_hash.expect("Missing m_replay_compatibility_hash from struct"),
m_ngdp_root_key_is_dev_data: m_ngdp_root_key_is_dev_data.expect("Missing m_ngdp_root_key_is_dev_data from struct"),
}))
}}
...
#[tracing::instrument(name="89720::ReplayTrackerEEventId::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
...
pub fn parse_m_score_value_minerals_collection_rate(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_collection_rate) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_collection_rate).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_collection_rate(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_collection_rate) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_collection_rate).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_workers_active_count(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_workers_active_count) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_workers_active_count).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_in_progress_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_in_progress_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_in_progress_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_in_progress_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_in_progress_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_in_progress_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_in_progress_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_in_progress_technology) = tagged_vlq_int(input)?;

                      tracing::debug!("res: {:?}", m_score_value_minerals_used_in_progress_technology);
        Ok((tail, i32::try_from(m_score_value_minerals_used_in_progress_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_in_progress_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_in_progress_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_in_progress_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_in_progress_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_in_progress_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_in_progress_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_in_progress_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_in_progress_technology) = tagged_vlq_int(input)?;

                      tracing::debug!("res: {:?}", m_score_value_vespene_used_in_progress_technology);
        Ok((tail, i32::try_from(m_score_value_vespene_used_in_progress_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_current_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_current_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_current_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_current_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_current_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_current_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_used_current_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_current_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_current_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_current_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_current_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_current_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_current_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_current_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_current_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_current_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_current_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_current_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_lost_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_lost_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_lost_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_lost_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_lost_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_lost_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_lost_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_lost_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_lost_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_lost_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_lost_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_lost_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_lost_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_lost_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_lost_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_lost_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_lost_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_lost_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_killed_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_killed_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_killed_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_killed_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_killed_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_killed_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_killed_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_killed_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_killed_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_killed_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_killed_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_killed_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_killed_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_killed_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_killed_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_killed_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_killed_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_killed_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
...
pub fn parse_m_score_value_minerals_used_active_forces(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_used_active_forces) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_used_active_forces).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_used_active_forces(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_used_active_forces) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_used_active_forces).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_friendly_fire_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_friendly_fire_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_friendly_fire_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_friendly_fire_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_friendly_fire_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_friendly_fire_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_minerals_friendly_fire_technology(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_minerals_friendly_fire_technology) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_minerals_friendly_fire_technology).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_friendly_fire_army(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_friendly_fire_army) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_friendly_fire_army).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_friendly_fire_economy(input: &[u8]) -> IResult<&[u8], i32> {
 let (tail, m_score_value_vespene_friendly_fire_economy) = tagged_vlq_int(input)?;
        ...
        Ok((tail, i32::try_from(m_score_value_vespene_friendly_fire_economy).unwrap()))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse_m_score_value_vespene_friendly_fire_technology(input: &[u8]) -> IResult<&[u8], i32> {
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSPlayerStats::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
match field_tag {
 0 => {
tracing::debug!("Field [{i}]: tagged '0' for field m_score_value_minerals_current");
 if m_score_value_minerals_current.is_none() {
let (new_tail, parsed_m_score_value_minerals_current) = Self::parse_m_score_value_minerals_current(tail)?;
tail = new_tail;
m_score_value_minerals_current = Some(parsed_m_score_value_minerals_current);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 1 => {
tracing::debug!("Field [{i}]: tagged '1' for field m_score_value_vespene_current");
 if m_score_value_vespene_current.is_none() {
let (new_tail, parsed_m_score_value_vespene_current) = Self::parse_m_score_value_vespene_current(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_score_value_minerals_collection_rate) = Self::parse_m_score_value_minerals_collection_rate(tail)?;
tail = new_tail;
m_score_value_minerals_collection_rate = Some(parsed_m_score_value_minerals_collection_rate);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_collection_rate) = Self::parse_m_score_value_vespene_collection_rate(tail)?;
tail = new_tail;
m_score_value_vespene_collection_rate = Some(parsed_m_score_value_vespene_collection_rate);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 4 => {
tracing::debug!("Field [{i}]: tagged '4' for field m_score_value_workers_active_count");
 if m_score_value_workers_active_count.is_none() {
let (new_tail, parsed_m_score_value_workers_active_count) = Self::parse_m_score_value_workers_active_count(tail)?;
tail = new_tail;
m_score_value_workers_active_count = Some(parsed_m_score_value_workers_active_count);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_in_progress_army) = Self::parse_m_score_value_minerals_used_in_progress_army(tail)?;
tail = new_tail;
m_score_value_minerals_used_in_progress_army = Some(parsed_m_score_value_minerals_used_in_progress_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_in_progress_economy) = Self::parse_m_score_value_minerals_used_in_progress_economy(tail)?;
tail = new_tail;
m_score_value_minerals_used_in_progress_economy = Some(parsed_m_score_value_minerals_used_in_progress_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_in_progress_technology) = Self::parse_m_score_value_minerals_used_in_progress_technology(tail)?;
tail = new_tail;
m_score_value_minerals_used_in_progress_technology = Some(parsed_m_score_value_minerals_used_in_progress_technology);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_in_progress_army) = Self::parse_m_score_value_vespene_used_in_progress_army(tail)?;
tail = new_tail;
m_score_value_vespene_used_in_progress_army = Some(parsed_m_score_value_vespene_used_in_progress_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_in_progress_economy) = Self::parse_m_score_value_vespene_used_in_progress_economy(tail)?;
tail = new_tail;
m_score_value_vespene_used_in_progress_economy = Some(parsed_m_score_value_vespene_used_in_progress_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_in_progress_technology) = Self::parse_m_score_value_vespene_used_in_progress_technology(tail)?;
tail = new_tail;
m_score_value_vespene_used_in_progress_technology = Some(parsed_m_score_value_vespene_used_in_progress_technology);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_current_army) = Self::parse_m_score_value_minerals_used_current_army(tail)?;
tail = new_tail;
m_score_value_minerals_used_current_army = Some(parsed_m_score_value_minerals_used_current_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_current_economy) = Self::parse_m_score_value_minerals_used_current_economy(tail)?;
tail = new_tail;
m_score_value_minerals_used_current_economy = Some(parsed_m_score_value_minerals_used_current_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_used_current_technology) = Self::parse_m_score_value_minerals_used_current_technology(tail)?;
tail = new_tail;
m_score_value_minerals_used_current_technology = Some(parsed_m_score_value_minerals_used_current_technology);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_current_army) = Self::parse_m_score_value_vespene_used_current_army(tail)?;
tail = new_tail;
m_score_value_vespene_used_current_army = Some(parsed_m_score_value_vespene_used_current_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_current_economy) = Self::parse_m_score_value_vespene_used_current_economy(tail)?;
tail = new_tail;
m_score_value_vespene_used_current_economy = Some(parsed_m_score_value_vespene_used_current_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_current_technology) = Self::parse_m_score_value_vespene_used_current_technology(tail)?;
tail = new_tail;
m_score_value_vespene_used_current_technology = Some(parsed_m_score_value_vespene_used_current_technology);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 17 => {
tracing::debug!("Field [{i}]: tagged '17' for field m_score_value_minerals_lost_army");
 if m_score_value_minerals_lost_army.is_none() {
let (new_tail, parsed_m_score_value_minerals_lost_army) = Self::parse_m_score_value_minerals_lost_army(tail)?;
tail = new_tail;
m_score_value_minerals_lost_army = Some(parsed_m_score_value_minerals_lost_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_lost_economy) = Self::parse_m_score_value_minerals_lost_economy(tail)?;
tail = new_tail;
m_score_value_minerals_lost_economy = Some(parsed_m_score_value_minerals_lost_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_lost_technology) = Self::parse_m_score_value_minerals_lost_technology(tail)?;
tail = new_tail;
m_score_value_minerals_lost_technology = Some(parsed_m_score_value_minerals_lost_technology);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 20 => {
tracing::debug!("Field [{i}]: tagged '20' for field m_score_value_vespene_lost_army");
 if m_score_value_vespene_lost_army.is_none() {
let (new_tail, parsed_m_score_value_vespene_lost_army) = Self::parse_m_score_value_vespene_lost_army(tail)?;
tail = new_tail;
m_score_value_vespene_lost_army = Some(parsed_m_score_value_vespene_lost_army);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 21 => {
tracing::debug!("Field [{i}]: tagged '21' for field m_score_value_vespene_lost_economy");
 if m_score_value_vespene_lost_economy.is_none() {
let (new_tail, parsed_m_score_value_vespene_lost_economy) = Self::parse_m_score_value_vespene_lost_economy(tail)?;
tail = new_tail;
m_score_value_vespene_lost_economy = Some(parsed_m_score_value_vespene_lost_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_lost_technology) = Self::parse_m_score_value_vespene_lost_technology(tail)?;
tail = new_tail;
m_score_value_vespene_lost_technology = Some(parsed_m_score_value_vespene_lost_technology);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 23 => {
tracing::debug!("Field [{i}]: tagged '23' for field m_score_value_minerals_killed_army");
 if m_score_value_minerals_killed_army.is_none() {
let (new_tail, parsed_m_score_value_minerals_killed_army) = Self::parse_m_score_value_minerals_killed_army(tail)?;
tail = new_tail;
m_score_value_minerals_killed_army = Some(parsed_m_score_value_minerals_killed_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_killed_economy) = Self::parse_m_score_value_minerals_killed_economy(tail)?;
tail = new_tail;
m_score_value_minerals_killed_economy = Some(parsed_m_score_value_minerals_killed_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_killed_technology) = Self::parse_m_score_value_minerals_killed_technology(tail)?;
tail = new_tail;
m_score_value_minerals_killed_technology = Some(parsed_m_score_value_minerals_killed_technology);
continue;
} else {
...
panic!("Unhandled duplicate field.");
 ...
 26 => {
tracing::debug!("Field [{i}]: tagged '26' for field m_score_value_vespene_killed_army");
 if m_score_value_vespene_killed_army.is_none() {
let (new_tail, parsed_m_score_value_vespene_killed_army) = Self::parse_m_score_value_vespene_killed_army(tail)?;
tail = new_tail;
m_score_value_vespene_killed_army = Some(parsed_m_score_value_vespene_killed_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_killed_economy) = Self::parse_m_score_value_vespene_killed_economy(tail)?;
tail = new_tail;
m_score_value_vespene_killed_economy = Some(parsed_m_score_value_vespene_killed_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_killed_technology) = Self::parse_m_score_value_vespene_killed_technology(tail)?;
tail = new_tail;
m_score_value_vespene_killed_technology = Some(parsed_m_score_value_vespene_killed_technology);
continue;
} else {
...
let (new_tail, parsed_m_score_value_food_used) = Self::parse_m_score_value_food_used(tail)?;
tail = new_tail;
m_score_value_food_used = Some(parsed_m_score_value_food_used);
continue;
} else {
tracing::error!("Field m_score_value_food_used with tag 29 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 30 => {
...
let (new_tail, parsed_m_score_value_food_made) = Self::parse_m_score_value_food_made(tail)?;
tail = new_tail;
m_score_value_food_made = Some(parsed_m_score_value_food_made);
continue;
} else {
tracing::error!("Field m_score_value_food_made with tag 30 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 31 => {
...
let (new_tail, parsed_m_score_value_minerals_used_active_forces) = Self::parse_m_score_value_minerals_used_active_forces(tail)?;
tail = new_tail;
m_score_value_minerals_used_active_forces = Some(parsed_m_score_value_minerals_used_active_forces);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_used_active_forces) = Self::parse_m_score_value_vespene_used_active_forces(tail)?;
tail = new_tail;
m_score_value_vespene_used_active_forces = Some(parsed_m_score_value_vespene_used_active_forces);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_friendly_fire_army) = Self::parse_m_score_value_minerals_friendly_fire_army(tail)?;
tail = new_tail;
m_score_value_minerals_friendly_fire_army = Some(parsed_m_score_value_minerals_friendly_fire_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_friendly_fire_economy) = Self::parse_m_score_value_minerals_friendly_fire_economy(tail)?;
tail = new_tail;
m_score_value_minerals_friendly_fire_economy = Some(parsed_m_score_value_minerals_friendly_fire_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_minerals_friendly_fire_technology) = Self::parse_m_score_value_minerals_friendly_fire_technology(tail)?;
tail = new_tail;
m_score_value_minerals_friendly_fire_technology = Some(parsed_m_score_value_minerals_friendly_fire_technology);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_friendly_fire_army) = Self::parse_m_score_value_vespene_friendly_fire_army(tail)?;
tail = new_tail;
m_score_value_vespene_friendly_fire_army = Some(parsed_m_score_value_vespene_friendly_fire_army);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_friendly_fire_economy) = Self::parse_m_score_value_vespene_friendly_fire_economy(tail)?;
tail = new_tail;
m_score_value_vespene_friendly_fire_economy = Some(parsed_m_score_value_vespene_friendly_fire_economy);
continue;
} else {
...
let (new_tail, parsed_m_score_value_vespene_friendly_fire_technology) = Self::parse_m_score_value_vespene_friendly_fire_technology(tail)?;
tail = new_tail;
m_score_value_vespene_friendly_fire_technology = Some(parsed_m_score_value_vespene_friendly_fire_technology);
continue;
} else {
...
}Ok((tail, Self {m_score_value_minerals_current: m_score_value_minerals_current.expect("Missing m_score_value_minerals_current from struct"),
m_score_value_vespene_current: m_score_value_vespene_current.expect("Missing m_score_value_vespene_current from struct"),
m_score_value_minerals_collection_rate: m_score_value_minerals_collection_rate.expect("Missing m_score_value_minerals_collection_rate from struct"),
m_score_value_vespene_collection_rate: m_score_value_vespene_collection_rate.expect("Missing m_score_value_vespene_collection_rate from struct"),
m_score_value_workers_active_count: m_score_value_workers_active_count.expect("Missing m_score_value_workers_active_count from struct"),
m_score_value_minerals_used_in_progress_army: m_score_value_minerals_used_in_progress_army.expect("Missing m_score_value_minerals_used_in_progress_army from struct"),
m_score_value_minerals_used_in_progress_economy: m_score_value_minerals_used_in_progress_economy.expect("Missing m_score_value_minerals_used_in_progress_economy from struct"),
m_score_value_minerals_used_in_progress_technology: m_score_value_minerals_used_in_progress_technology.expect("Missing m_score_value_minerals_used_in_progress_technology from struct"),
m_score_value_vespene_used_in_progress_army: m_score_value_vespene_used_in_progress_army.expect("Missing m_score_value_vespene_used_in_progress_army from struct"),
m_score_value_vespene_used_in_progress_economy: m_score_value_vespene_used_in_progress_economy.expect("Missing m_score_value_vespene_used_in_progress_economy from struct"),
m_score_value_vespene_used_in_progress_technology: m_score_value_vespene_used_in_progress_technology.expect("Missing m_score_value_vespene_used_in_progress_technology from struct"),
m_score_value_minerals_used_current_army: m_score_value_minerals_used_current_army.expect("Missing m_score_value_minerals_used_current_army from struct"),
m_score_value_minerals_used_current_economy: m_score_value_minerals_used_current_economy.expect("Missing m_score_value_minerals_used_current_economy from struct"),
m_score_value_minerals_used_current_technology: m_score_value_minerals_used_current_technology.expect("Missing m_score_value_minerals_used_current_technology from struct"),
m_score_value_vespene_used_current_army: m_score_value_vespene_used_current_army.expect("Missing m_score_value_vespene_used_current_army from struct"),
m_score_value_vespene_used_current_economy: m_score_value_vespene_used_current_economy.expect("Missing m_score_value_vespene_used_current_economy from struct"),
m_score_value_vespene_used_current_technology: m_score_value_vespene_used_current_technology.expect("Missing m_score_value_vespene_used_current_technology from struct"),
m_score_value_minerals_lost_army: m_score_value_minerals_lost_army.expect("Missing m_score_value_minerals_lost_army from struct"),
m_score_value_minerals_lost_economy: m_score_value_minerals_lost_economy.expect("Missing m_score_value_minerals_lost_economy from struct"),
m_score_value_minerals_lost_technology: m_score_value_minerals_lost_technology.expect("Missing m_score_value_minerals_lost_technology from struct"),
m_score_value_vespene_lost_army: m_score_value_vespene_lost_army.expect("Missing m_score_value_vespene_lost_army from struct"),
m_score_value_vespene_lost_economy: m_score_value_vespene_lost_economy.expect("Missing m_score_value_vespene_lost_economy from struct"),
m_score_value_vespene_lost_technology: m_score_value_vespene_lost_technology.expect("Missing m_score_value_vespene_lost_technology from struct"),
m_score_value_minerals_killed_army: m_score_value_minerals_killed_army.expect("Missing m_score_value_minerals_killed_army from struct"),
m_score_value_minerals_killed_economy: m_score_value_minerals_killed_economy.expect("Missing m_score_value_minerals_killed_economy from struct"),
m_score_value_minerals_killed_technology: m_score_value_minerals_killed_technology.expect("Missing m_score_value_minerals_killed_technology from struct"),
m_score_value_vespene_killed_army: m_score_value_vespene_killed_army.expect("Missing m_score_value_vespene_killed_army from struct"),
m_score_value_vespene_killed_economy: m_score_value_vespene_killed_economy.expect("Missing m_score_value_vespene_killed_economy from struct"),
m_score_value_vespene_killed_technology: m_score_value_vespene_killed_technology.expect("Missing m_score_value_vespene_killed_technology from struct"),
m_score_value_food_used: m_score_value_food_used.expect("Missing m_score_value_food_used from struct"),
m_score_value_food_made: m_score_value_food_made.expect("Missing m_score_value_food_made from struct"),
m_score_value_minerals_used_active_forces: m_score_value_minerals_used_active_forces.expect("Missing m_score_value_minerals_used_active_forces from struct"),
m_score_value_vespene_used_active_forces: m_score_value_vespene_used_active_forces.expect("Missing m_score_value_vespene_used_active_forces from struct"),
m_score_value_minerals_friendly_fire_army: m_score_value_minerals_friendly_fire_army.expect("Missing m_score_value_minerals_friendly_fire_army from struct"),
m_score_value_minerals_friendly_fire_economy: m_score_value_minerals_friendly_fire_economy.expect("Missing m_score_value_minerals_friendly_fire_economy from struct"),
m_score_value_minerals_friendly_fire_technology: m_score_value_minerals_friendly_fire_technology.expect("Missing m_score_value_minerals_friendly_fire_technology from struct"),
m_score_value_vespene_friendly_fire_army: m_score_value_vespene_friendly_fire_army.expect("Missing m_score_value_vespene_friendly_fire_army from struct"),
m_score_value_vespene_friendly_fire_economy: m_score_value_vespene_friendly_fire_economy.expect("Missing m_score_value_vespene_friendly_fire_economy from struct"),
m_score_value_vespene_friendly_fire_technology: m_score_value_vespene_friendly_fire_technology.expect("Missing m_score_value_vespene_friendly_fire_technology from struct"),
}))
}}
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSPlayerStatsEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
}Ok((tail, Self {m_player_id: m_player_id.expect("Missing m_player_id from struct"),
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitBornEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
let (new_tail, parsed_m_unit_tag_index) = Self::parse_m_unit_tag_index(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_unit_tag_recycle) = Self::parse_m_unit_tag_recycle(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_unit_type_name) = Self::parse_m_unit_type_name(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_control_player_id) = Self::parse_m_control_player_id(tail)?;
tail = new_tail;
m_control_player_id = Some(parsed_m_control_player_id);
continue;
} else {
tracing::error!("Field m_control_player_id with tag 3 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 4 => {
...
let (new_tail, parsed_m_upkeep_player_id) = Self::parse_m_upkeep_player_id(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_creator_unit_tag_index) = Self::parse_m_creator_unit_tag_index(tail)?;
tail = new_tail;
m_creator_unit_tag_index = Some(parsed_m_creator_unit_tag_index);
continue;
} else {
tracing::error!("Field m_creator_unit_tag_index with tag 7 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 8 => {
...
let (new_tail, parsed_m_creator_unit_tag_recycle) = Self::parse_m_creator_unit_tag_recycle(tail)?;
tail = new_tail;
m_creator_unit_tag_recycle = Some(parsed_m_creator_unit_tag_recycle);
continue;
} else {
tracing::error!("Field m_creator_unit_tag_recycle with tag 8 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 9 => {
...
let (new_tail, parsed_m_creator_ability_name) = Self::parse_m_creator_ability_name(tail)?;
tail = new_tail;
m_creator_ability_name = Some(parsed_m_creator_ability_name);
continue;
} else {
tracing::error!("Field m_creator_ability_name with tag 9 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
...
}Ok((tail, Self {m_unit_tag_index: m_unit_tag_index.expect("Missing m_unit_tag_index from struct"),
m_unit_tag_recycle: m_unit_tag_recycle.expect("Missing m_unit_tag_recycle from struct"),
m_unit_type_name: m_unit_type_name.expect("Missing m_unit_type_name from struct"),
m_control_player_id: m_control_player_id.expect("Missing m_control_player_id from struct"),
m_upkeep_player_id: m_upkeep_player_id.expect("Missing m_upkeep_player_id from struct"),
m_x: m_x.expect("Missing m_x from struct"),
m_y: m_y.expect("Missing m_y from struct"),
m_creator_unit_tag_index: m_creator_unit_tag_index.expect("Missing m_creator_unit_tag_index from struct"),
m_creator_unit_tag_recycle: m_creator_unit_tag_recycle.expect("Missing m_creator_unit_tag_recycle from struct"),
m_creator_ability_name: m_creator_ability_name.expect("Missing m_creator_ability_name from struct"),
}))
}}
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitDiedEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_unit_tag_index: Option<u32> = None;
...
let (new_tail, parsed_m_killer_player_id) = Self::parse_m_killer_player_id(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_killer_unit_tag_index) = Self::parse_m_killer_unit_tag_index(tail)?;
tail = new_tail;
m_killer_unit_tag_index = Some(parsed_m_killer_unit_tag_index);
continue;
} else {
tracing::error!("Field m_killer_unit_tag_index with tag 5 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 6 => {
...
let (new_tail, parsed_m_killer_unit_tag_recycle) = Self::parse_m_killer_unit_tag_recycle(tail)?;
tail = new_tail;
m_killer_unit_tag_recycle = Some(parsed_m_killer_unit_tag_recycle);
continue;
} else {
tracing::error!("Field m_killer_unit_tag_recycle with tag 6 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
...
m_unit_tag_recycle: m_unit_tag_recycle.expect("Missing m_unit_tag_recycle from struct"),
m_killer_player_id: m_killer_player_id.expect("Missing m_killer_player_id from struct"),
m_x: m_x.expect("Missing m_x from struct"),
m_y: m_y.expect("Missing m_y from struct"),
m_killer_unit_tag_index: m_killer_unit_tag_index.expect("Missing m_killer_unit_tag_index from struct"),
m_killer_unit_tag_recycle: m_killer_unit_tag_recycle.expect("Missing m_killer_unit_tag_recycle from struct"),
}))
}}
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitOwnerChangeEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_unit_tag_index: Option<u32> = None;
...
tracing::error!("Field m_control_player_id with tag 2 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 3 => {
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitTypeChangeEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_unit_tag_index: Option<u32> = None;
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUpgradeEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_player_id: Option<u8> = None;
...
let (new_tail, parsed_m_upgrade_type_name) = Self::parse_m_upgrade_type_name(tail)?;
tail = new_tail;
m_upgrade_type_name = Some(parsed_m_upgrade_type_name);
continue;
} else {
tracing::error!("Field m_upgrade_type_name with tag 1 was already provided");
panic!("Unhandled duplicate field.");
                        }
},
 2 => {
...
m_upgrade_type_name: m_upgrade_type_name.expect("Missing m_upgrade_type_name from struct"),
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitInitEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_unit_tag_index: Option<u32> = None;
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitDoneEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_unit_tag_index: Option<u32> = None;
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSUnitPositionsEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
...
let (new_tail, parsed_m_first_unit_index) = Self::parse_m_first_unit_index(tail)?;
tail = new_tail;
...
}Ok((tail, Self {m_first_unit_index: m_first_unit_index.expect("Missing m_first_unit_index from struct"),
...
#[tracing::instrument(name="89720::byte_aligned::ReplayTrackerSPlayerSetupEvent::Parse", level = "debug", skip(input), fields(peek = peek_hex(input)))]
pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
let (tail, _) = validate_struct_tag(input)?;
let (mut tail, struct_field_count) = parse_vlq_int(tail)?;
let mut m_player_id: Option<u8> = None;
...
pub struct CFilePath {
    pub value: Vec<u8>,}
impl CFilePath {
#[tracing::instrument(name="89720::CFilePath::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
...
pub struct TRaceId {
    pub value: i64,}
impl TRaceId {
#[tracing::instrument(name="89720::TRaceId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct TRaceCount {
    pub value: i64,}
impl TRaceCount {
#[tracing::instrument(name="89720::TRaceCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 1;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::bit_packed::TRacePreference::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_race: m_race.expect("Missing m_race from struct"),
}))
}}
...
pub struct CAllowedRaces {
    pub value: Vec<u8>,}
impl CAllowedRaces {
#[tracing::instrument(name="89720::CAllowedRaces::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct Int8 {
    pub value: i64,}
impl Int8 {
#[tracing::instrument(name="89720::Int8::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = -128;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct Int16 {
    pub value: i64,}
impl Int16 {
#[tracing::instrument(name="89720::Int16::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct Int32 {
    pub value: i64,}
impl Int32 {
#[tracing::instrument(name="89720::Int32::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct Int64 {
    pub value: i64,}
impl Int64 {
#[tracing::instrument(name="89720::Int64::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct Uint8 {
    pub value: i64,}
impl Uint8 {
#[tracing::instrument(name="89720::Uint8::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct Uint16 {
    pub value: i64,}
impl Uint16 {
#[tracing::instrument(name="89720::Uint16::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 16;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct Uint32 {
    pub value: i64,}
impl Uint32 {
#[tracing::instrument(name="89720::Uint32::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 32;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct Uint64 {
    pub value: i64,}
impl Uint64 {
#[tracing::instrument(name="89720::Uint64::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 64;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct Uint6 {
    pub value: i64,}
impl Uint6 {
#[tracing::instrument(name="89720::Uint6::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct Uint14 {
    pub value: i64,}
impl Uint14 {
#[tracing::instrument(name="89720::Uint14::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct Uint22 {
    pub value: i64,}
impl Uint22 {
#[tracing::instrument(name="89720::Uint22::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
impl SVarUint32 {
#[tracing::instrument(name="89720::SVarUint32::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// ChoiceType:
...
pub struct TUserId {
    pub value: i64,}
impl TUserId {
#[tracing::instrument(name="89720::TUserId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct TUserCount {
    pub value: i64,}
impl TUserCount {
#[tracing::instrument(name="89720::TUserCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct CCacheHandle {
    pub value: Vec<u8>,}
impl CCacheHandle {
#[tracing::instrument(name="89720::CCacheHandle::BlobType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct CUserName {
    pub value: Vec<u8>,}
impl CUserName {
#[tracing::instrument(name="89720::CUserName::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct CClanTag {
    pub value: Vec<u8>,}
impl CClanTag {
#[tracing::instrument(name="89720::CClanTag::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 8;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
pub struct CHeroHandle {
    pub value: Vec<u8>,}
impl CHeroHandle {
#[tracing::instrument(name="89720::CHeroHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct CSkinHandle {
    pub value: Vec<u8>,}
impl CSkinHandle {
#[tracing::instrument(name="89720::CSkinHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 10;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
pub struct CMountHandle {
    pub value: Vec<u8>,}
impl CMountHandle {
#[tracing::instrument(name="89720::CMountHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 10;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::CArtifactHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 10;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
pub struct CToonHandle {
    pub value: Vec<u8>,}
impl CToonHandle {
#[tracing::instrument(name="89720::CToonHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::CCommanderHandle::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 10;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
impl EObserve {
#[tracing::instrument(name="89720::EObserve::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 3
let num_bits: usize = 2;
...
#[tracing::instrument(name="89720::CAllowedObserveTypes::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::bit_packed::TTeamPreference::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_team: m_team.expect("Missing m_team from struct"),
}))
}}
...
let (tail, res) = CClanTag::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_clan_tag);
Ok((tail, m_clan_tag))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_clan_logo(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<CCacheHandle>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_highest_league(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint8>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = Uint8::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_highest_league);
Ok((tail, m_highest_league))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_combined_race_levels(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint32>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = Uint32::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_combined_race_levels);
Ok((tail, m_combined_race_levels))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_random_seed(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_random_seed) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_random_seed);
        Ok((tail, m_random_seed))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_race_preference(input: (&[u8], usize)) -> IResult<(&[u8], usize), TRacePreference> {
 let (tail, m_race_preference) = TRacePreference::parse(input)?;
    tracing::debug!("res: {:?}", m_race_preference);
        Ok((tail, m_race_preference))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_team_preference(input: (&[u8], usize)) -> IResult<(&[u8], usize), TTeamPreference> {
 let (tail, m_team_preference) = TTeamPreference::parse(input)?;
    tracing::debug!("res: {:?}", m_team_preference);
        Ok((tail, m_team_preference))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_test_map(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_test_map) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_test_map);
        Ok((tail, m_test_map))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_test_auto(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_test_auto) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_test_auto);
        Ok((tail, m_test_auto))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_examine(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_examine) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_examine);
        Ok((tail, m_examine))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_custom_interface(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_custom_interface) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_custom_interface);
        Ok((tail, m_custom_interface))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_test_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_test_type) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_test_type);
        Ok((tail, m_test_type))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_hero(input: (&[u8], usize)) -> IResult<(&[u8], usize), CHeroHandle> {
 let (tail, m_hero) = CHeroHandle::parse(input)?;
    tracing::debug!("res: {:?}", m_hero);
        Ok((tail, m_hero))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_skin(input: (&[u8], usize)) -> IResult<(&[u8], usize), CSkinHandle> {
 let (tail, m_skin) = CSkinHandle::parse(input)?;
    tracing::debug!("res: {:?}", m_skin);
        Ok((tail, m_skin))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mount(input: (&[u8], usize)) -> IResult<(&[u8], usize), CMountHandle> {
 let (tail, m_mount) = CMountHandle::parse(input)?;
    tracing::debug!("res: {:?}", m_mount);
        Ok((tail, m_mount))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_toon_handle(input: (&[u8], usize)) -> IResult<(&[u8], usize), CToonHandle> {
 let (tail, m_toon_handle) = CToonHandle::parse(input)?;
    tracing::debug!("res: {:?}", m_toon_handle);
        Ok((tail, m_toon_handle))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_scaled_rating(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Int32>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::SUserInitialData::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_combined_race_levels) = Self::parse_m_combined_race_levels(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_name: m_name.expect("Missing m_name from struct"),
m_clan_tag: m_clan_tag.expect("Missing m_clan_tag from struct"),
m_clan_logo: m_clan_logo.expect("Missing m_clan_logo from struct"),
m_highest_league: m_highest_league.expect("Missing m_highest_league from struct"),
m_combined_race_levels: m_combined_race_levels.expect("Missing m_combined_race_levels from struct"),
m_random_seed: m_random_seed.expect("Missing m_random_seed from struct"),
m_race_preference: m_race_preference.expect("Missing m_race_preference from struct"),
m_team_preference: m_team_preference.expect("Missing m_team_preference from struct"),
m_test_map: m_test_map.expect("Missing m_test_map from struct"),
m_test_auto: m_test_auto.expect("Missing m_test_auto from struct"),
m_examine: m_examine.expect("Missing m_examine from struct"),
m_custom_interface: m_custom_interface.expect("Missing m_custom_interface from struct"),
m_test_type: m_test_type.expect("Missing m_test_type from struct"),
...
m_scaled_rating: m_scaled_rating.expect("Missing m_scaled_rating from struct"),
}))
}}
...
#[tracing::instrument(name="89720::CUserInitialDataArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 5;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(SUserInitialData::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
impl ELeaveReason {
#[tracing::instrument(name="89720::ELeaveReason::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 16
let num_bits: usize = 4;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::EReconnectStatus::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
impl SVersion {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_flags) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_flags);
        Ok((tail, m_flags))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_major(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_major) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_major);
        Ok((tail, m_major))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_minor(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_minor) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_minor);
        Ok((tail, m_minor))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_revision(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_revision) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_revision);
        Ok((tail, m_revision))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_build(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_build) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_build);
        Ok((tail, m_build))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_base_build(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_base_build) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_base_build);
        Ok((tail, m_base_build))
}
#[tracing::instrument(name="89720::bit_packed::SVersion::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
impl Smd5 {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_data_deprecated(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Vec<Uint8>>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::Smd5::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
pub struct GameTColorId {
    pub value: i64,}
impl GameTColorId {
#[tracing::instrument(name="89720::GameTColorId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 5;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTColorCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 6;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_color(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTColorId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::GameTColorPreference::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_color: m_color.expect("Missing m_color from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameCAllowedColors::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameESynchronous::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 3
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameESynthesized::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 2
...
impl GameEDebug {
#[tracing::instrument(name="89720::GameEDebug::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 2
let num_bits: usize = 1;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEHijackMethod::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 2
let num_bits: usize = 1;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
pub struct GameTQueryId {
    pub value: Uint16,}
impl GameTQueryId {
#[tracing::instrument(name="89720::GameTQueryId::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
impl GameEEventId {
#[tracing::instrument(name="89720::GameEEventId::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 120
...
tracing::debug!("Variant ETriggerMercenaryPanelPurchase for value '78'");
 ...
 79 => {
tracing::debug!("Variant ETriggerMercenaryPanelSelectionChanged for value '79'");
...
tracing::debug!("Variant ETriggerBattleReportSelectionChanged for value '84'");
                ...
                let (tail, res) = GameSTriggerBattleReportPanelSelectionChangedEvent::parse(tail)?;
...
tracing::debug!("Variant ETriggerMouseWheel for value '92'");
 ...
 93 => {
tracing::debug!("Variant ETriggerPurchasePanelSelectedPurchaseItemChanged for value '93'");

                let (tail, res) = GameSTriggerPurchasePanelSelectedPurchaseItemChangedEvent::parse(tail)?;
                Ok((tail, Self::ETriggerPurchasePanelSelectedPurchaseItemChanged(res)))
},
 94 => {
tracing::debug!("Variant ETriggerPurchasePanelSelectedPurchaseCategoryChanged for value '94'");

                let (tail, res) = GameSTriggerPurchasePanelSelectedPurchaseCategoryChangedEvent::parse(tail)?;
                Ok((tail, Self::ETriggerPurchasePanelSelectedPurchaseCategoryChanged(res)))
},
 95 => {
...
tracing::debug!("Variant ETriggerCutsceneConversationLine for value '99'");
 ...
 100 => {
tracing::debug!("Variant ETriggerCutsceneConversationLineMissing for value '100'");

                let (tail, res) = GameSTriggerCutsceneConversationLineMissingEvent::parse(tail)?;
...
impl GameSCmdAbil {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_abil_link(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTAbilLink> {
 let (tail, m_abil_link) = GameTAbilLink::parse(input)?;
    tracing::debug!("res: {:?}", m_abil_link);
        Ok((tail, m_abil_link))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_abil_cmd_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint8>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::GameSCmdAbil::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_abil_link: m_abil_link.expect("Missing m_abil_link from struct"),
m_abil_cmd_index: m_abil_cmd_index.expect("Missing m_abil_cmd_index from struct"),
m_abil_cmd_data: m_abil_cmd_data.expect("Missing m_abil_cmd_data from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_unit_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint16> {
 let (tail, m_target_unit_flags) = Uint16::parse(input)?;
    tracing::debug!("res: {:?}", m_target_unit_flags);
        Ok((tail, m_target_unit_flags))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_timer(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_timer) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_timer);
        Ok((tail, m_timer))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_tag(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitTag> {
 let (tail, m_tag) = GameTUnitTag::parse(input)?;
    tracing::debug!("res: {:?}", m_tag);
        Ok((tail, m_tag))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_snapshot_unit_link(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitLink> {
 let (tail, m_snapshot_unit_link) = GameTUnitLink::parse(input)?;
    tracing::debug!("res: {:?}", m_snapshot_unit_link);
        Ok((tail, m_snapshot_unit_link))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_snapshot_control_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTPlayerId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTPlayerId::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_snapshot_control_player_id);
Ok((tail, m_snapshot_control_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_snapshot_upkeep_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTPlayerId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTPlayerId::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_snapshot_upkeep_player_id);
Ok((tail, m_snapshot_upkeep_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_snapshot_point(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSMapCoord3D> {
 let (tail, m_snapshot_point) = GameSMapCoord3D::parse(input)?;
    tracing::debug!("res: {:?}", m_snapshot_point);
        Ok((tail, m_snapshot_point))
}
#[tracing::instrument(name="89720::bit_packed::GameSCmdDataTargetUnit::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_snapshot_unit_link) = Self::parse_m_snapshot_unit_link(tail)?;
tail = new_tail;
m_snapshot_unit_link = Some(parsed_m_snapshot_unit_link);
}
 if let Some(None) = m_snapshot_control_player_id  {
let (new_tail, parsed_m_snapshot_control_player_id) = Self::parse_m_snapshot_control_player_id(tail)?;
tail = new_tail;
m_snapshot_control_player_id = Some(parsed_m_snapshot_control_player_id);
}
 if let Some(None) = m_snapshot_upkeep_player_id  {
let (new_tail, parsed_m_snapshot_upkeep_player_id) = Self::parse_m_snapshot_upkeep_player_id(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_target_unit_flags: m_target_unit_flags.expect("Missing m_target_unit_flags from struct"),
m_timer: m_timer.expect("Missing m_timer from struct"),
m_tag: m_tag.expect("Missing m_tag from struct"),
m_snapshot_unit_link: m_snapshot_unit_link.expect("Missing m_snapshot_unit_link from struct"),
m_snapshot_control_player_id: m_snapshot_control_player_id.expect("Missing m_snapshot_control_player_id from struct"),
m_snapshot_upkeep_player_id: m_snapshot_upkeep_player_id.expect("Missing m_snapshot_upkeep_player_id from struct"),
m_snapshot_point: m_snapshot_point.expect("Missing m_snapshot_point from struct"),
}))
}}
...
impl GameSCmdData {
#[tracing::instrument(name="89720::GameSCmdData::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// ChoiceType:
            // Use the number of elements in the json .fields to calculate how many
            // bits to have unique tags.
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_slot_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTLobbySlotId> {
 let (tail, m_slot_id) = GameTLobbySlotId::parse(input)?;
    tracing::debug!("res: {:?}", m_slot_id);
        Ok((tail, m_slot_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_slot_change(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSLobbySlotChange> {
 let (tail, m_slot_change) = GameSLobbySlotChange::parse(input)?;
    tracing::debug!("res: {:?}", m_slot_change);
        Ok((tail, m_slot_change))
}
#[tracing::instrument(name="89720::bit_packed::GameSSetLobbySlotEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_slot_id: m_slot_id.expect("Missing m_slot_id from struct"),
m_slot_change: m_slot_change.expect("Missing m_slot_change from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_drop_session_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserId> {
 let (tail, m_drop_session_user_id) = TUserId::parse(input)?;
    tracing::debug!("res: {:?}", m_drop_session_user_id);
        Ok((tail, m_drop_session_user_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_reason(input: (&[u8], usize)) -> IResult<(&[u8], usize), ELeaveReason> {
 let (tail, m_reason) = ELeaveReason::parse(input)?;
    tracing::debug!("res: {:?}", m_reason);
        Ok((tail, m_reason))
}
#[tracing::instrument(name="89720::bit_packed::GameSDropUserEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_drop_session_user_id) = Self::parse_m_drop_session_user_id(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_drop_session_user_id: m_drop_session_user_id.expect("Missing m_drop_session_user_id from struct"),
m_reason: m_reason.expect("Missing m_reason from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSStartGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSDropOurselvesEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSBankFileEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSBankSectionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_name: Option<Vec<u8>> = None;
 if m_name.is_none() {
...
pub fn parse_m_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSBankKeyEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_name: Option<Vec<u8>> = None;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSBankValueEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_type: Option<Uint32> = None;
let mut m_name: Option<Vec<u8>> = None;
...
Ok((tail, Self {m_type: m_type.expect("Missing m_type from struct"),
...
#[tracing::instrument(name="89720::bit_packed::GameSBankSignatureEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_fully_downloaded(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_game_fully_downloaded) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_game_fully_downloaded);
        Ok((tail, m_game_fully_downloaded))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_development_cheats_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_development_cheats_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_development_cheats_enabled);
        Ok((tail, m_development_cheats_enabled))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_test_cheats_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_test_cheats_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_test_cheats_enabled);
        Ok((tail, m_test_cheats_enabled))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_multiplayer_cheats_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_multiplayer_cheats_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_multiplayer_cheats_enabled);
        Ok((tail, m_multiplayer_cheats_enabled))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sync_checksumming_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_sync_checksumming_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_sync_checksumming_enabled);
        Ok((tail, m_sync_checksumming_enabled))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_is_map_to_map_transition(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_is_map_to_map_transition) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_is_map_to_map_transition);
        Ok((tail, m_is_map_to_map_transition))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_debug_pause_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_debug_pause_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_debug_pause_enabled);
        Ok((tail, m_debug_pause_enabled))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_use_galaxy_asserts(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_use_galaxy_asserts) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_use_galaxy_asserts);
        Ok((tail, m_use_galaxy_asserts))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_platform_mac(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_platform_mac) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_platform_mac);
        Ok((tail, m_platform_mac))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_camera_follow(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_camera_follow) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_camera_follow);
        Ok((tail, m_camera_follow))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_base_build_num(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_base_build_num) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_base_build_num);
        Ok((tail, m_base_build_num))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_build_num(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_build_num) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_build_num);
        Ok((tail, m_build_num))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_version_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_version_flags) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_version_flags);
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSUserOptionsEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_game_fully_downloaded: m_game_fully_downloaded.expect("Missing m_game_fully_downloaded from struct"),
m_development_cheats_enabled: m_development_cheats_enabled.expect("Missing m_development_cheats_enabled from struct"),
m_test_cheats_enabled: m_test_cheats_enabled.expect("Missing m_test_cheats_enabled from struct"),
m_multiplayer_cheats_enabled: m_multiplayer_cheats_enabled.expect("Missing m_multiplayer_cheats_enabled from struct"),
m_sync_checksumming_enabled: m_sync_checksumming_enabled.expect("Missing m_sync_checksumming_enabled from struct"),
m_is_map_to_map_transition: m_is_map_to_map_transition.expect("Missing m_is_map_to_map_transition from struct"),
m_debug_pause_enabled: m_debug_pause_enabled.expect("Missing m_debug_pause_enabled from struct"),
m_use_galaxy_asserts: m_use_galaxy_asserts.expect("Missing m_use_galaxy_asserts from struct"),
m_platform_mac: m_platform_mac.expect("Missing m_platform_mac from struct"),
m_camera_follow: m_camera_follow.expect("Missing m_camera_follow from struct"),
m_base_build_num: m_base_build_num.expect("Missing m_base_build_num from struct"),
m_build_num: m_build_num.expect("Missing m_build_num from struct"),
m_version_flags: m_version_flags.expect("Missing m_version_flags from struct"),
m_hotkey_profile: m_hotkey_profile.expect("Missing m_hotkey_profile from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_picked_map_tag(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_picked_map_tag) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_picked_map_tag);
        Ok((tail, m_picked_map_tag))
}
#[tracing::instrument(name="89720::bit_packed::GameSPickMapTagEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_picked_map_tag: m_picked_map_tag.expect("Missing m_picked_map_tag from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSUserFinishedLoadingEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSUserFinishedLoadingSyncEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_duration(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_game_duration) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_game_duration);
        Ok((tail, m_game_duration))
}
#[tracing::instrument(name="89720::bit_packed::GameSSetGameDurationEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_game_duration: m_game_duration.expect("Missing m_game_duration from struct"),
}))
}}
...
impl GameSTurnEvent {
#[tracing::instrument(name="89720::bit_packed::GameSTurnEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSPointMini> {
 let (tail, m_target) = GameSPointMini::parse(input)?;
    tracing::debug!("res: {:?}", m_target);
        Ok((tail, m_target))
}
#[tracing::instrument(name="89720::bit_packed::GameSCameraSaveEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_which: m_which.expect("Missing m_which from struct"),
m_target: m_target.expect("Missing m_target from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_pause_type_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_pause_type_index) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_pause_type_index);
        Ok((tail, m_pause_type_index))
}
#[tracing::instrument(name="89720::bit_packed::GameSPauseGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_pause_type_index: m_pause_type_index.expect("Missing m_pause_type_index from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSUnpauseGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_pause_type_index: Option<Uint8> = None;
 if m_pause_type_index.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSSingleStepGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_speed(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEGameSpeed> {
 let (tail, m_speed) = GameEGameSpeed::parse(input)?;
    tracing::debug!("res: {:?}", m_speed);
        Ok((tail, m_speed))
}
#[tracing::instrument(name="89720::bit_packed::GameSSetGameSpeedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_speed: m_speed.expect("Missing m_speed from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_delta(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_delta) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_delta);
        Ok((tail, m_delta))
}
#[tracing::instrument(name="89720::bit_packed::GameSAddGameSpeedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_delta: m_delta.expect("Missing m_delta from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_replay_jump_game_loop(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint32>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::GameSReplayJumpEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_replay_jump_game_loop: Option<Option<Uint32>> = Some(None);
 if let Some(None) = m_replay_jump_game_loop  {
let (new_tail, parsed_m_replay_jump_game_loop) = Self::parse_m_replay_jump_game_loop(tail)?;
tail = new_tail;
m_replay_jump_game_loop = Some(parsed_m_replay_jump_game_loop);
}
Ok((tail, Self {m_replay_jump_game_loop: m_replay_jump_game_loop.expect("Missing m_replay_jump_game_loop from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_file_name(input: (&[u8], usize)) -> IResult<(&[u8], usize), CFilePath> {
 let (tail, m_file_name) = CFilePath::parse(input)?;
    tracing::debug!("res: {:?}", m_file_name);
        Ok((tail, m_file_name))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_automatic(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_automatic) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_automatic);
        Ok((tail, m_automatic))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_overwrite(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_overwrite) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_overwrite);
        Ok((tail, m_overwrite))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_name(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<u8>> {
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSSaveGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_file_name: m_file_name.expect("Missing m_file_name from struct"),
m_automatic: m_automatic.expect("Missing m_automatic from struct"),
m_overwrite: m_overwrite.expect("Missing m_overwrite from struct"),
m_name: m_name.expect("Missing m_name from struct"),
...
#[tracing::instrument(name="89720::bit_packed::GameSSaveGameDoneEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSLoadGameDoneEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_point(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSPoint> {
 let (tail, m_point) = GameSPoint::parse(input)?;
    tracing::debug!("res: {:?}", m_point);
        Ok((tail, m_point))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_time(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_time) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_time);
        Ok((tail, m_time))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_verb(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCCheatString> {
 let (tail, m_verb) = GameCCheatString::parse(input)?;
    tracing::debug!("res: {:?}", m_verb);
        Ok((tail, m_verb))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_arguments(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCCheatString> {
 let (tail, m_arguments) = GameCCheatString::parse(input)?;
    tracing::debug!("res: {:?}", m_arguments);
        Ok((tail, m_arguments))
}
#[tracing::instrument(name="89720::bit_packed::GameSCheatEventData::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_point: m_point.expect("Missing m_point from struct"),
m_time: m_time.expect("Missing m_time from struct"),
m_verb: m_verb.expect("Missing m_verb from struct"),
m_arguments: m_arguments.expect("Missing m_arguments from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSCheatEventData> {
 let (tail, m_data) = GameSCheatEventData::parse(input)?;
    tracing::debug!("res: {:?}", m_data);
        Ok((tail, m_data))
}
#[tracing::instrument(name="89720::bit_packed::GameSSessionCheatEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_data: m_data.expect("Missing m_data from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sequence(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_sequence) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_sequence);
        Ok((tail, m_sequence))
}
#[tracing::instrument(name="89720::bit_packed::GameSCommandManagerResetEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_sequence: m_sequence.expect("Missing m_sequence from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSGameCheatEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_data: Option<GameSCheatEventData> = None;
 if m_data.is_none() {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_abil(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameSCmdAbil>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSCmdData> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_other_unit(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTUnitTag>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_group(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint32>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::GameSCmdEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_cmd_flags: m_cmd_flags.expect("Missing m_cmd_flags from struct"),
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control_group_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTControlGroupId> {
 let (tail, m_control_group_id) = GameTControlGroupId::parse(input)?;
    tracing::debug!("res: {:?}", m_control_group_id);
        Ok((tail, m_control_group_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_delta(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSelectionDelta> {
 let (tail, m_delta) = GameSSelectionDelta::parse(input)?;
    tracing::debug!("res: {:?}", m_delta);
        Ok((tail, m_delta))
}
#[tracing::instrument(name="89720::bit_packed::GameSSelectionDeltaEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_control_group_id: m_control_group_id.expect("Missing m_control_group_id from struct"),
m_delta: m_delta.expect("Missing m_delta from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control_group_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTControlGroupIndex> {
 let (tail, m_control_group_index) = GameTControlGroupIndex::parse(input)?;
    tracing::debug!("res: {:?}", m_control_group_index);
        Ok((tail, m_control_group_index))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control_group_update(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEControlGroupUpdate> {
 let (tail, m_control_group_update) = GameEControlGroupUpdate::parse(input)?;
    tracing::debug!("res: {:?}", m_control_group_update);
        Ok((tail, m_control_group_update))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mask(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSelectionMask> {
 let (tail, m_mask) = GameSSelectionMask::parse(input)?;
    tracing::debug!("res: {:?}", m_mask);
        Ok((tail, m_mask))
}
#[tracing::instrument(name="89720::bit_packed::GameSControlGroupUpdateEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_control_group_index) = Self::parse_m_control_group_index(tail)?;
tail = new_tail;
m_control_group_index = Some(parsed_m_control_group_index);
}
 if m_control_group_update.is_none() {
let (new_tail, parsed_m_control_group_update) = Self::parse_m_control_group_update(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_control_group_index: m_control_group_index.expect("Missing m_control_group_index from struct"),
m_control_group_update: m_control_group_update.expect("Missing m_control_group_update from struct"),
m_mask: m_mask.expect("Missing m_mask from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_selection_sync_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSelectionSyncData> {
...
#[tracing::instrument(name="89720::bit_packed::GameSSelectionSyncCheckEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_control_group_id: Option<GameTControlGroupId> = None;
...
let (new_tail, parsed_m_selection_sync_data) = Self::parse_m_selection_sync_data(tail)?;
tail = new_tail;
m_selection_sync_data = Some(parsed_m_selection_sync_data);
}
Ok((tail, Self {m_control_group_id: m_control_group_id.expect("Missing m_control_group_id from struct"),
m_selection_sync_data: m_selection_sync_data.expect("Missing m_selection_sync_data from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_recipient_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTPlayerId> {
 let (tail, m_recipient_id) = GameTPlayerId::parse(input)?;
    tracing::debug!("res: {:?}", m_recipient_id);
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSResourceTradeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_recipient_id: m_recipient_id.expect("Missing m_recipient_id from struct"),
m_resources: m_resources.expect("Missing m_resources from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_chat_message(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCTriggerChatMessageString> {
 let (tail, m_chat_message) = GameCTriggerChatMessageString::parse(input)?;
    tracing::debug!("res: {:?}", m_chat_message);
        Ok((tail, m_chat_message))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerChatMessageEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_chat_message: m_chat_message.expect("Missing m_chat_message from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_beacon(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_beacon) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_beacon);
        Ok((tail, m_beacon))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_ally(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_ally) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_ally);
        Ok((tail, m_ally))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_flags) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_flags);
        Ok((tail, m_flags))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_build(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_build) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_build);
        Ok((tail, m_build))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_unit_tag(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitTag> {
 let (tail, m_target_unit_tag) = GameTUnitTag::parse(input)?;
    tracing::debug!("res: {:?}", m_target_unit_tag);
        Ok((tail, m_target_unit_tag))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_unit_snapshot_unit_link(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitLink> {
 let (tail, m_target_unit_snapshot_unit_link) = GameTUnitLink::parse(input)?;
    tracing::debug!("res: {:?}", m_target_unit_snapshot_unit_link);
        Ok((tail, m_target_unit_snapshot_unit_link))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_unit_snapshot_upkeep_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_target_unit_snapshot_upkeep_player_id) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_target_unit_snapshot_upkeep_player_id);
        Ok((tail, m_target_unit_snapshot_upkeep_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_unit_snapshot_control_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_target_unit_snapshot_control_player_id) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_target_unit_snapshot_control_player_id);
        Ok((tail, m_target_unit_snapshot_control_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_point(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSPoint3> {
 let (tail, m_target_point) = GameSPoint3::parse(input)?;
    tracing::debug!("res: {:?}", m_target_point);
        Ok((tail, m_target_point))
}
#[tracing::instrument(name="89720::bit_packed::GameSaiCommunicateEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_target_unit_snapshot_unit_link) = Self::parse_m_target_unit_snapshot_unit_link(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_target_unit_snapshot_upkeep_player_id) = Self::parse_m_target_unit_snapshot_upkeep_player_id(tail)?;
tail = new_tail;
m_target_unit_snapshot_upkeep_player_id = Some(parsed_m_target_unit_snapshot_upkeep_player_id);
}
 if m_target_unit_snapshot_control_player_id.is_none() {
let (new_tail, parsed_m_target_unit_snapshot_control_player_id) = Self::parse_m_target_unit_snapshot_control_player_id(tail)?;
tail = new_tail;
m_target_unit_snapshot_control_player_id = Some(parsed_m_target_unit_snapshot_control_player_id);
}
...
Ok((tail, Self {m_beacon: m_beacon.expect("Missing m_beacon from struct"),
m_ally: m_ally.expect("Missing m_ally from struct"),
m_flags: m_flags.expect("Missing m_flags from struct"),
m_build: m_build.expect("Missing m_build from struct"),
m_target_unit_tag: m_target_unit_tag.expect("Missing m_target_unit_tag from struct"),
m_target_unit_snapshot_unit_link: m_target_unit_snapshot_unit_link.expect("Missing m_target_unit_snapshot_unit_link from struct"),
m_target_unit_snapshot_upkeep_player_id: m_target_unit_snapshot_upkeep_player_id.expect("Missing m_target_unit_snapshot_upkeep_player_id from struct"),
m_target_unit_snapshot_control_player_id: m_target_unit_snapshot_control_player_id.expect("Missing m_target_unit_snapshot_control_player_id from struct"),
m_target_point: m_target_point.expect("Missing m_target_point from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSSetAbsoluteGameSpeedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_speed: Option<GameEGameSpeed> = None;
 if m_speed.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSAddAbsoluteGameSpeedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_delta: Option<Int8> = None;
 if m_delta.is_none() {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitTag> {
 let (tail, m_unit) = GameTUnitTag::parse(input)?;
    tracing::debug!("res: {:?}", m_unit);
        Ok((tail, m_unit))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_link(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitLink> {
 let (tail, m_unit_link) = GameTUnitLink::parse(input)?;
    tracing::debug!("res: {:?}", m_unit_link);
        Ok((tail, m_unit_link))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_control_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTPlayerId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTPlayerId::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_unit_control_player_id);
Ok((tail, m_unit_control_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_upkeep_player_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTPlayerId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTPlayerId::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_unit_upkeep_player_id);
Ok((tail, m_unit_upkeep_player_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_position(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSMapCoord3D> {
 let (tail, m_unit_position) = GameSMapCoord3D::parse(input)?;
    tracing::debug!("res: {:?}", m_unit_position);
        Ok((tail, m_unit_position))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_is_under_construction(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_unit_is_under_construction) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_unit_is_under_construction);
        Ok((tail, m_unit_is_under_construction))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_pinged_minimap(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_pinged_minimap) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_pinged_minimap);
        Ok((tail, m_pinged_minimap))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_option(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_option) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_option);
        Ok((tail, m_option))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPingEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_point: Option<GameSPoint> = None;
...
let (new_tail, parsed_m_unit_control_player_id) = Self::parse_m_unit_control_player_id(tail)?;
tail = new_tail;
m_unit_control_player_id = Some(parsed_m_unit_control_player_id);
}
 if let Some(None) = m_unit_upkeep_player_id  {
let (new_tail, parsed_m_unit_upkeep_player_id) = Self::parse_m_unit_upkeep_player_id(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_unit_is_under_construction) = Self::parse_m_unit_is_under_construction(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_point: m_point.expect("Missing m_point from struct"),
m_unit: m_unit.expect("Missing m_unit from struct"),
m_unit_link: m_unit_link.expect("Missing m_unit_link from struct"),
m_unit_control_player_id: m_unit_control_player_id.expect("Missing m_unit_control_player_id from struct"),
m_unit_upkeep_player_id: m_unit_upkeep_player_id.expect("Missing m_unit_upkeep_player_id from struct"),
m_unit_position: m_unit_position.expect("Missing m_unit_position from struct"),
m_unit_is_under_construction: m_unit_is_under_construction.expect("Missing m_unit_is_under_construction from struct"),
m_pinged_minimap: m_pinged_minimap.expect("Missing m_pinged_minimap from struct"),
m_option: m_option.expect("Missing m_option from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSBroadcastCheatEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_verb: Option<GameCCheatString> = None;
let mut m_arguments: Option<GameCCheatString> = None;
...
Ok((tail, Self {m_verb: m_verb.expect("Missing m_verb from struct"),
m_arguments: m_arguments.expect("Missing m_arguments from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_alliance(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_alliance) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_alliance);
        Ok((tail, m_alliance))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_control) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_control);
        Ok((tail, m_control))
}
#[tracing::instrument(name="89720::bit_packed::GameSAllianceEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_alliance: m_alliance.expect("Missing m_alliance from struct"),
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_tag(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTUnitTag> {
 let (tail, m_unit_tag) = GameTUnitTag::parse(input)?;
    tracing::debug!("res: {:?}", m_unit_tag);
        Ok((tail, m_unit_tag))
}
#[tracing::instrument(name="89720::bit_packed::GameSUnitClickEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_unit_tag: m_unit_tag.expect("Missing m_unit_tag from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSUnitHighlightEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_unit_tag: Option<GameTUnitTag> = None;
let mut m_flags: Option<Uint8> = None;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_conversation_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_conversation_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_conversation_id);
        Ok((tail, m_conversation_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_reply_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_reply_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_reply_id);
        Ok((tail, m_reply_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerReplySelectedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_conversation_id: m_conversation_id.expect("Missing m_conversation_id from struct"),
m_reply_id: m_reply_id.expect("Missing m_reply_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_session_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserId> {
 let (tail, m_session_user_id) = TUserId::parse(input)?;
    tracing::debug!("res: {:?}", m_session_user_id);
        Ok((tail, m_session_user_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_add_new_game_user(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_add_new_game_user) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_add_new_game_user);
        Ok((tail, m_add_new_game_user))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserId> {
 let (tail, m_game_user_id) = TUserId::parse(input)?;
    tracing::debug!("res: {:?}", m_game_user_id);
        Ok((tail, m_game_user_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSHijackReplaySessionUserInfo::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_session_user_id: m_session_user_id.expect("Missing m_session_user_id from struct"),
m_add_new_game_user: m_add_new_game_user.expect("Missing m_add_new_game_user from struct"),
m_game_user_id: m_game_user_id.expect("Missing m_game_user_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_user_infos(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<GameSHijackReplaySessionUserInfo>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 5)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
pub fn parse_m_method(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEHijackMethod> {
 let (tail, m_method) = GameEHijackMethod::parse(input)?;
    tracing::debug!("res: {:?}", m_method);
        Ok((tail, m_method))
}
#[tracing::instrument(name="89720::bit_packed::GameSHijackReplaySessionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_user_infos: m_user_infos.expect("Missing m_user_infos from struct"),
m_method: m_method.expect("Missing m_method from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSHijackReplayGameUserInfo::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_game_user_id: Option<TUserId> = None;
let mut m_observe: Option<EObserve> = None;
...
Ok((tail, Self {m_game_user_id: m_game_user_id.expect("Missing m_game_user_id from struct"),
m_observe: m_observe.expect("Missing m_observe from struct"),
m_name: m_name.expect("Missing m_name from struct"),
m_toon_handle: m_toon_handle.expect("Missing m_toon_handle from struct"),
m_clan_tag: m_clan_tag.expect("Missing m_clan_tag from struct"),
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_user_infos(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<GameSHijackReplayGameUserInfo>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 5)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
#[tracing::instrument(name="89720::bit_packed::GameSHijackReplayGameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerAbortMissionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_purchase_item_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_purchase_item_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_purchase_item_id);
        Ok((tail, m_purchase_item_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPurchaseMadeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_purchase_item_id: m_purchase_item_id.expect("Missing m_purchase_item_id from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPurchaseExitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_difficulty_level(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_difficulty_level) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_difficulty_level);
        Ok((tail, m_difficulty_level))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetMissionLaunchedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_difficulty_level: m_difficulty_level.expect("Missing m_difficulty_level from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetPanelCanceledEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_control_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_control_id);
        Ok((tail, m_control_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_event_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_event_type) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_event_type);
        Ok((tail, m_event_type))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_event_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), MEventData> {
 let (tail, m_event_data) = MEventData::parse(input)?;
    tracing::debug!("res: {:?}", m_event_data);
        Ok((tail, m_event_data))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerDialogControlEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_control_id: m_control_id.expect("Missing m_control_id from struct"),
m_event_type: m_event_type.expect("Missing m_event_type from struct"),
m_event_data: m_event_data.expect("Missing m_event_data from struct"),
}))
}}
...
impl MEventData {
#[tracing::instrument(name="89720::MEventData::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// ChoiceType:
            // Use the number of elements in the json .fields to calculate how many
            // bits to have unique tags.
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerSkippedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sound_hash(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_sound_hash) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_sound_hash);
        Ok((tail, m_sound_hash))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_length(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_length) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_length);
        Ok((tail, m_length))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerSoundLengthQueryEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_sound_hash: m_sound_hash.expect("Missing m_sound_hash from struct"),
m_length: m_length.expect("Missing m_length from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sync_info(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSyncSoundLength> {
 let (tail, m_sync_info) = GameSSyncSoundLength::parse(input)?;
    tracing::debug!("res: {:?}", m_sync_info);
        Ok((tail, m_sync_info))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerSoundLengthSyncEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_sync_info: m_sync_info.expect("Missing m_sync_info from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_query_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTQueryId> {
 let (tail, m_query_id) = GameTQueryId::parse(input)?;
    tracing::debug!("res: {:?}", m_query_id);
        Ok((tail, m_query_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_length_ms(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_length_ms) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_length_ms);
        Ok((tail, m_length_ms))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_finish_game_loop(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_finish_game_loop) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_finish_game_loop);
        Ok((tail, m_finish_game_loop))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerAnimLengthQueryByNameEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_query_id: m_query_id.expect("Missing m_query_id from struct"),
m_length_ms: m_length_ms.expect("Missing m_length_ms from struct"),
m_finish_game_loop: m_finish_game_loop.expect("Missing m_finish_game_loop from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerAnimLengthQueryByPropsEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_query_id: Option<GameTQueryId> = None;
let mut m_length_ms: Option<Uint32> = None;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_anim_wait_query_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTQueryId> {
 let (tail, m_anim_wait_query_id) = GameTQueryId::parse(input)?;
    tracing::debug!("res: {:?}", m_anim_wait_query_id);
        Ok((tail, m_anim_wait_query_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerAnimOffsetEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_anim_wait_query_id: Option<GameTQueryId> = None;
 if m_anim_wait_query_id.is_none() {
let (new_tail, parsed_m_anim_wait_query_id) = Self::parse_m_anim_wait_query_id(tail)?;
tail = new_tail;
m_anim_wait_query_id = Some(parsed_m_anim_wait_query_id);
}
Ok((tail, Self {m_anim_wait_query_id: m_anim_wait_query_id.expect("Missing m_anim_wait_query_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sound(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTTriggerSoundTag> {
 let (tail, m_sound) = GameTTriggerSoundTag::parse(input)?;
    tracing::debug!("res: {:?}", m_sound);
        Ok((tail, m_sound))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerSoundOffsetEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_sound: m_sound.expect("Missing m_sound from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_transmission_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_transmission_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_transmission_id);
        Ok((tail, m_transmission_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_thread(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTTriggerThreadTag> {
 let (tail, m_thread) = GameTTriggerThreadTag::parse(input)?;
    tracing::debug!("res: {:?}", m_thread);
        Ok((tail, m_thread))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerTransmissionOffsetEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_transmission_id: m_transmission_id.expect("Missing m_transmission_id from struct"),
m_thread: m_thread.expect("Missing m_thread from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerTransmissionCompleteEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_transmission_id: Option<Int32> = None;
 if m_transmission_id.is_none() {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameSPointMini>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_distance(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTFixedMiniBitsUnsigned>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTFixedMiniBitsUnsigned::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_distance);
Ok((tail, m_distance))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_pitch(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTFixedMiniBitsUnsigned>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = GameTFixedMiniBitsUnsigned::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_pitch);
Ok((tail, m_pitch))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_yaw(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameTFixedMiniBitsUnsigned>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_follow(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_follow) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_follow);
        Ok((tail, m_follow))
}
#[tracing::instrument(name="89720::bit_packed::GameSCameraUpdateEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_target: m_target.expect("Missing m_target from struct"),
m_distance: m_distance.expect("Missing m_distance from struct"),
m_pitch: m_pitch.expect("Missing m_pitch from struct"),
m_yaw: m_yaw.expect("Missing m_yaw from struct"),
m_reason: m_reason.expect("Missing m_reason from struct"),
m_follow: m_follow.expect("Missing m_follow from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_skip_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEConversationSkip> {
 let (tail, m_skip_type) = GameEConversationSkip::parse(input)?;
    tracing::debug!("res: {:?}", m_skip_type);
        Ok((tail, m_skip_type))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerConversationSkippedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_skip_type: m_skip_type.expect("Missing m_skip_type from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_button(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_button) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_button);
        Ok((tail, m_button))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_down(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_down) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_down);
        Ok((tail, m_down))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_pos_ui(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSuiCoord> {
 let (tail, m_pos_ui) = GameSuiCoord::parse(input)?;
    tracing::debug!("res: {:?}", m_pos_ui);
        Ok((tail, m_pos_ui))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_pos_world(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSMapCoord3D> {
 let (tail, m_pos_world) = GameSMapCoord3D::parse(input)?;
    tracing::debug!("res: {:?}", m_pos_world);
        Ok((tail, m_pos_world))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMouseClickedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_button: m_button.expect("Missing m_button from struct"),
m_down: m_down.expect("Missing m_down from struct"),
m_pos_ui: m_pos_ui.expect("Missing m_pos_ui from struct"),
m_pos_world: m_pos_world.expect("Missing m_pos_world from struct"),
m_flags: m_flags.expect("Missing m_flags from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMouseMovedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_pos_ui: Option<GameSuiCoord> = None;
let mut m_pos_world: Option<GameSMapCoord3D> = None;
...
Ok((tail, Self {m_pos_ui: m_pos_ui.expect("Missing m_pos_ui from struct"),
m_pos_world: m_pos_world.expect("Missing m_pos_world from struct"),
m_flags: m_flags.expect("Missing m_flags from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_achievement_link(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTAchievementLink> {
...
#[tracing::instrument(name="89720::bit_packed::GameSAchievementAwardedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_achievement_link: m_achievement_link.expect("Missing m_achievement_link from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_hotkey(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_hotkey) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_hotkey);
        Ok((tail, m_hotkey))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_down(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerHotkeyPressedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_hotkey: m_hotkey.expect("Missing m_hotkey from struct"),
m_down: m_down.expect("Missing m_down from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_state(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_state) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_state);
        Ok((tail, m_state))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerTargetModeUpdateEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_abil_link: Option<GameTAbilLink> = None;
let mut m_abil_cmd_index: Option<i64> = None;
...
Ok((tail, Self {m_abil_link: m_abil_link.expect("Missing m_abil_link from struct"),
m_abil_cmd_index: m_abil_cmd_index.expect("Missing m_abil_cmd_index from struct"),
m_state: m_state.expect("Missing m_state from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetPanelReplayEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_soundtrack(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_soundtrack) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_soundtrack);
        Ok((tail, m_soundtrack))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerSoundtrackDoneEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_soundtrack: m_soundtrack.expect("Missing m_soundtrack from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_planet_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_planet_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_planet_id);
        Ok((tail, m_planet_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetMissionSelectedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_planet_id: m_planet_id.expect("Missing m_planet_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_key(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
 let (tail, m_key) = Int8::parse(input)?;
    tracing::debug!("res: {:?}", m_key);
        Ok((tail, m_key))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int8> {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerKeyPressedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_key: m_key.expect("Missing m_key from struct"),
m_flags: m_flags.expect("Missing m_flags from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetPanelBirthCompleteEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPlanetPanelDeathCompleteEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSResourceRequestEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_resources: Option<Vec<Int32>> = None;
 if m_resources.is_none() {
...
Ok((tail, Self {m_resources: m_resources.expect("Missing m_resources from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_fulfill_request_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_fulfill_request_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_fulfill_request_id);
        Ok((tail, m_fulfill_request_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSResourceRequestFulfillEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_fulfill_request_id: Option<Int32> = None;
 if m_fulfill_request_id.is_none() {
let (new_tail, parsed_m_fulfill_request_id) = Self::parse_m_fulfill_request_id(tail)?;
tail = new_tail;
m_fulfill_request_id = Some(parsed_m_fulfill_request_id);
}
Ok((tail, Self {m_fulfill_request_id: m_fulfill_request_id.expect("Missing m_fulfill_request_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_cancel_request_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_cancel_request_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_cancel_request_id);
        Ok((tail, m_cancel_request_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSResourceRequestCancelEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_cancel_request_id: m_cancel_request_id.expect("Missing m_cancel_request_id from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerResearchPanelExitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerResearchPanelPurchaseEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_error(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_error) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_error);
        Ok((tail, m_error))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_abil(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameSCmdAbil>> {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerCommandErrorEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_error: m_error.expect("Missing m_error from struct"),
m_abil: m_abil.expect("Missing m_abil from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_research_item_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_research_item_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_research_item_id);
        Ok((tail, m_research_item_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerResearchPanelSelectionChangedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_research_item_id: m_research_item_id.expect("Missing m_research_item_id from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMercenaryPanelExitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMercenaryPanelPurchaseEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mercenary_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_mercenary_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_mercenary_id);
        Ok((tail, m_mercenary_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMercenaryPanelSelectionChangedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_mercenary_id: m_mercenary_id.expect("Missing m_mercenary_id from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerVictoryPanelExitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerBattleReportPanelExitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_battle_report_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerBattleReportPanelPlayMissionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_battle_report_id: m_battle_report_id.expect("Missing m_battle_report_id from struct"),
m_difficulty_level: m_difficulty_level.expect("Missing m_difficulty_level from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerBattleReportPanelPlaySceneEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_battle_report_id: Option<Int32> = None;
 if m_battle_report_id.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerBattleReportPanelSelectionChangedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_battle_report_id: Option<Int32> = None;
 if m_battle_report_id.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerVictoryPanelPlayMissionAgainEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_difficulty_level: Option<Int32> = None;
 if m_difficulty_level.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMovieStartedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMovieFinishedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_decrement_seconds(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_decrement_seconds) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_decrement_seconds);
        Ok((tail, m_decrement_seconds))
}
#[tracing::instrument(name="89720::bit_packed::GameSDecrementGameTimeRemainingEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_decrement_seconds: m_decrement_seconds.expect("Missing m_decrement_seconds from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_portrait_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_portrait_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_portrait_id);
        Ok((tail, m_portrait_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPortraitLoadedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_portrait_id: m_portrait_id.expect("Missing m_portrait_id from struct"),
}))
}}
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSTriggerMovieFunctionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_function_name: m_function_name.expect("Missing m_function_name from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_result(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_result) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_result);
        Ok((tail, m_result))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerCustomDialogDismissedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_result: m_result.expect("Missing m_result from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_menu_item_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_game_menu_item_index) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_game_menu_item_index);
        Ok((tail, m_game_menu_item_index))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerGameMenuItemSelectedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_game_menu_item_index: Option<Int32> = None;
 if m_game_menu_item_index.is_none() {
let (new_tail, parsed_m_game_menu_item_index) = Self::parse_m_game_menu_item_index(tail)?;
tail = new_tail;
m_game_menu_item_index = Some(parsed_m_game_menu_item_index);
}
Ok((tail, Self {m_game_menu_item_index: m_game_menu_item_index.expect("Missing m_game_menu_item_index from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_wheel_spin(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedMiniBitsSigned> {
 let (tail, m_wheel_spin) = GameTFixedMiniBitsSigned::parse(input)?;
    tracing::debug!("res: {:?}", m_wheel_spin);
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerMouseWheelEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_wheel_spin: m_wheel_spin.expect("Missing m_wheel_spin from struct"),
m_flags: m_flags.expect("Missing m_flags from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPurchasePanelSelectedPurchaseItemChangedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_purchase_item_id: Option<Int32> = None;
 if m_purchase_item_id.is_none() {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_purchase_category_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_purchase_category_id) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_purchase_category_id);
        Ok((tail, m_purchase_category_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerPurchasePanelSelectedPurchaseCategoryChangedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_purchase_category_id: Option<Int32> = None;
 if m_purchase_category_id.is_none() {
let (new_tail, parsed_m_purchase_category_id) = Self::parse_m_purchase_category_id(tail)?;
tail = new_tail;
m_purchase_category_id = Some(parsed_m_purchase_category_id);
}
Ok((tail, Self {m_purchase_category_id: m_purchase_category_id.expect("Missing m_purchase_category_id from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_button(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTButtonLink> {
 let (tail, m_button) = GameTButtonLink::parse(input)?;
    tracing::debug!("res: {:?}", m_button);
        Ok((tail, m_button))
}
#[tracing::instrument(name="89720::bit_packed::GameSTriggerButtonPressedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerGameCreditsFinishedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_cutscene_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSTriggerCutsceneBookmarkFiredEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_cutscene_id: m_cutscene_id.expect("Missing m_cutscene_id from struct"),
m_bookmark_name: m_bookmark_name.expect("Missing m_bookmark_name from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerCutsceneEndSceneFiredEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_cutscene_id: Option<Int32> = None;
 if m_cutscene_id.is_none() {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_conversation_line(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<u8>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 7)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
pub fn parse_m_alt_conversation_line(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<u8>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 7)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSTriggerCutsceneConversationLineEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_cutscene_id: Option<Int32> = None;
...
let (new_tail, parsed_m_alt_conversation_line) = Self::parse_m_alt_conversation_line(tail)?;
tail = new_tail;
m_alt_conversation_line = Some(parsed_m_alt_conversation_line);
}
Ok((tail, Self {m_cutscene_id: m_cutscene_id.expect("Missing m_cutscene_id from struct"),
m_conversation_line: m_conversation_line.expect("Missing m_conversation_line from struct"),
m_alt_conversation_line: m_alt_conversation_line.expect("Missing m_alt_conversation_line from struct"),
}))
}}
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSTriggerCutsceneConversationLineMissingEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_cutscene_id: Option<Int32> = None;
let mut m_conversation_line: Option<Vec<u8>> = None;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_leave_reason(input: (&[u8], usize)) -> IResult<(&[u8], usize), ELeaveReason> {
 let (tail, m_leave_reason) = ELeaveReason::parse(input)?;
    tracing::debug!("res: {:?}", m_leave_reason);
        Ok((tail, m_leave_reason))
}
#[tracing::instrument(name="89720::bit_packed::GameSGameUserLeaveEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_leave_reason: m_leave_reason.expect("Missing m_leave_reason from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSGameUserJoinEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_observe: Option<EObserve> = None;
let mut m_name: Option<CUserName> = None;
...
let (new_tail, parsed_m_hijack_clone_game_user_id) = Self::parse_m_hijack_clone_game_user_id(tail)?;
tail = new_tail;
m_hijack_clone_game_user_id = Some(parsed_m_hijack_clone_game_user_id);
}
Ok((tail, Self {m_observe: m_observe.expect("Missing m_observe from struct"),
m_name: m_name.expect("Missing m_name from struct"),
m_toon_handle: m_toon_handle.expect("Missing m_toon_handle from struct"),
m_clan_tag: m_clan_tag.expect("Missing m_clan_tag from struct"),
m_clan_logo: m_clan_logo.expect("Missing m_clan_logo from struct"),
m_hijack: m_hijack.expect("Missing m_hijack from struct"),
m_hijack_clone_game_user_id: m_hijack_clone_game_user_id.expect("Missing m_hijack_clone_game_user_id from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameECommandManagerState::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 3
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_state(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameECommandManagerState> {
...
#[tracing::instrument(name="89720::bit_packed::GameSCommandManagerStateEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_state: m_state.expect("Missing m_state from struct"),
m_sequence: m_sequence.expect("Missing m_sequence from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSMapCoord3D> {
 let (tail, m_target) = GameSMapCoord3D::parse(input)?;
    tracing::debug!("res: {:?}", m_target);
        Ok((tail, m_target))
}
#[tracing::instrument(name="89720::bit_packed::GameSCmdUpdateTargetPointEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSCmdDataTargetUnit> {
 let (tail, m_target) = GameSCmdDataTargetUnit::parse(input)?;
    tracing::debug!("res: {:?}", m_target);
        Ok((tail, m_target))
}
#[tracing::instrument(name="89720::bit_packed::GameSCmdUpdateTargetUnitEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_catalog(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_catalog) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_catalog);
        Ok((tail, m_catalog))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_entry(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint16> {
 let (tail, m_entry) = Uint16::parse(input)?;
    tracing::debug!("res: {:?}", m_entry);
        Ok((tail, m_entry))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSCatalogModifyEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_catalog: m_catalog.expect("Missing m_catalog from struct"),
m_entry: m_entry.expect("Missing m_entry from struct"),
m_field: m_field.expect("Missing m_field from struct"),
m_value: m_value.expect("Missing m_value from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_index) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_index);
        Ok((tail, m_index))
}
#[tracing::instrument(name="89720::bit_packed::GameSHeroTalentTreeSelectedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_index: m_index.expect("Missing m_index from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSTriggerProfilerLoggingFinishedEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_shown(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_shown) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_shown);
        Ok((tail, m_shown))
}
#[tracing::instrument(name="89720::bit_packed::GameSHeroTalentTreeSelectionPanelToggledEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_shown: m_shown.expect("Missing m_shown from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_target_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserId> {
 let (tail, m_target_user_id) = TUserId::parse(input)?;
    tracing::debug!("res: {:?}", m_target_user_id);
        Ok((tail, m_target_user_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_muted(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_muted) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_muted);
        Ok((tail, m_muted))
}
#[tracing::instrument(name="89720::bit_packed::GameSMuteChatEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_target_user_id: m_target_user_id.expect("Missing m_target_user_id from struct"),
m_muted: m_muted.expect("Missing m_muted from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_replay_jump_game_loop(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Int32>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
let (tail, m_replay_jump_game_loop) = if is_provided {
let (tail, res) = Int32::parse(tail)?;
    (tail, Some(res))
...
#[tracing::instrument(name="89720::bit_packed::GameSConvertToReplaySessionEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_sync_time(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_sync_time) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_sync_time);
        Ok((tail, m_sync_time))
}
#[tracing::instrument(name="89720::bit_packed::GameSSetSyncLoadingTimeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_sync_time: m_sync_time.expect("Missing m_sync_time from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSSetSyncPlayingTimeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_sync_time: Option<Uint32> = None;
 if m_sync_time.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSPeerSetSyncLoadingTimeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_sync_time: Option<Uint32> = None;
 if m_sync_time.is_none() {
...
#[tracing::instrument(name="89720::bit_packed::GameSPeerSetSyncPlayingTimeEvent::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_sync_time: Option<Uint32> = None;
 if m_sync_time.is_none() {
...
impl GameEGameSpeed {
#[tracing::instrument(name="89720::GameEGameSpeed::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 5
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
impl GameEPhase {
#[tracing::instrument(name="89720::GameEPhase::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 6
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEConversationSkip::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 2
let num_bits: usize = 1;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameCCheatString::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 11;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameCTriggerChatMessageString::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 11;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameTAchievementLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameTAchievementTermLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameTButtonLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTUnitLink {
    pub value: Uint16,}
impl GameTUnitLink {
#[tracing::instrument(name="89720::GameTUnitLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTUnitTag {
    pub value: Uint32,}
impl GameTUnitTag {
#[tracing::instrument(name="89720::GameTUnitTag::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameTTriggerThreadTag::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameTTriggerSoundTag::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTAbilLink {
    pub value: Uint16,}
impl GameTAbilLink {
#[tracing::instrument(name="89720::GameTAbilLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTFixedBits {
    pub value: Int32,}
impl GameTFixedBits {
#[tracing::instrument(name="89720::GameTFixedBits::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameTFixedMiniBitsUnsigned::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameTFixedMiniBitsSigned::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameTPlayerLogoIndex::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTFixedInt {
    pub value: i64,}
impl GameTFixedInt {
#[tracing::instrument(name="89720::GameTFixedInt::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
pub struct GameTFixedUInt {
    pub value: i64,}
impl GameTFixedUInt {
#[tracing::instrument(name="89720::GameTFixedUInt::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
#[tracing::instrument(name="89720::GameTMapCoordFixedBits::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 20;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct GameTuiCoordX {
    pub value: i64,}
impl GameTuiCoordX {
#[tracing::instrument(name="89720::GameTuiCoordX::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
pub struct GameTuiCoordY {
    pub value: i64,}
impl GameTuiCoordY {
#[tracing::instrument(name="89720::GameTuiCoordY::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 11;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct GameTHeroLink {
    pub value: Uint16,}
impl GameTHeroLink {
#[tracing::instrument(name="89720::GameTHeroLink::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
impl GameSPoint {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_x(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedBits> {
 let (tail, x) = GameTFixedBits::parse(input)?;
    tracing::debug!("res: {:?}", x);
        Ok((tail, x))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_y(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedBits> {
 let (tail, y) = GameTFixedBits::parse(input)?;
    tracing::debug!("res: {:?}", y);
        Ok((tail, y))
}
#[tracing::instrument(name="89720::bit_packed::GameSPoint::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {x: x.expect("Missing x from struct"),
y: y.expect("Missing y from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_z(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedBits> {
 let (tail, z) = GameTFixedBits::parse(input)?;
    tracing::debug!("res: {:?}", z);
        Ok((tail, z))
}
#[tracing::instrument(name="89720::bit_packed::GameSPoint3::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut x: Option<GameTFixedBits> = None;
let mut y: Option<GameTFixedBits> = None;
...
Ok((tail, Self {x: x.expect("Missing x from struct"),
y: y.expect("Missing y from struct"),
z: z.expect("Missing z from struct"),
}))
}}
...
impl GameSPointMini {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_x(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedMiniBitsUnsigned> {
 let (tail, x) = GameTFixedMiniBitsUnsigned::parse(input)?;
    tracing::debug!("res: {:?}", x);
        Ok((tail, x))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_y(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTFixedMiniBitsUnsigned> {
 let (tail, y) = GameTFixedMiniBitsUnsigned::parse(input)?;
    tracing::debug!("res: {:?}", y);
        Ok((tail, y))
}
#[tracing::instrument(name="89720::bit_packed::GameSPointMini::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
impl GameSMapCoord {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_x(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTMapCoordFixedBits> {
 let (tail, x) = GameTMapCoordFixedBits::parse(input)?;
    tracing::debug!("res: {:?}", x);
        Ok((tail, x))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_y(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTMapCoordFixedBits> {
 let (tail, y) = GameTMapCoordFixedBits::parse(input)?;
    tracing::debug!("res: {:?}", y);
        Ok((tail, y))
}
#[tracing::instrument(name="89720::bit_packed::GameSMapCoord::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(name="89720::bit_packed::GameSMapCoord3D::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut x: Option<GameTMapCoordFixedBits> = None;
let mut y: Option<GameTMapCoordFixedBits> = None;
...
impl GameSuiCoord {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_x(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTuiCoordX> {
 let (tail, x) = GameTuiCoordX::parse(input)?;
    tracing::debug!("res: {:?}", x);
        Ok((tail, x))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_y(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTuiCoordY> {
 let (tail, y) = GameTuiCoordY::parse(input)?;
    tracing::debug!("res: {:?}", y);
        Ok((tail, y))
}
#[tracing::instrument(name="89720::bit_packed::GameSuiCoord::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
pub struct GameTHandicap {
    pub value: Uint32,}
impl GameTHandicap {
#[tracing::instrument(name="89720::GameTHandicap::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameTDifficulty::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 6;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameCAllowedDifficulty::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let bitarray_length_bits: usize = 6;
         let (tail, bitarray_length) = take_n_bits_into_i64(input, bitarray_length_bits)?;
         let (tail, value) = take_bit_array(tail, bitarray_length as usize)?;
...
pub struct GameTaiBuild {
    pub value: i64,}
impl GameTaiBuild {
#[tracing::instrument(name="89720::GameTaiBuild::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameCAllowedAiBuild::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let bitarray_length_bits: usize = 8;
         let (tail, bitarray_length) = take_n_bits_into_i64(input, bitarray_length_bits)?;
         let (tail, value) = take_bit_array(tail, bitarray_length as usize)?;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSSyncSoundLength::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSThumbnail::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_file: m_file.expect("Missing m_file from struct"),
}))
}}
...
impl GameSColor {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_a(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_a) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_a);
        Ok((tail, m_a))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_r(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_r) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_r);
        Ok((tail, m_r))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_g(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_g) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_g);
        Ok((tail, m_g))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_b(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_b) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_b);
        Ok((tail, m_b))
}
#[tracing::instrument(name="89720::bit_packed::GameSColor::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_a: m_a.expect("Missing m_a from struct"),
...
#[tracing::instrument(name="89720::GameEResultDetails::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_region(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_region) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_region);
        Ok((tail, m_region))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_program_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<u8>> {
 let (tail, m_program_id) = take_fourcc(input)?;
    tracing::debug!("res: {:?}", m_program_id);
        Ok((tail, m_program_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_realm(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
pub fn parse_m_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint64> {
 let (tail, m_id) = Uint64::parse(input)?;
    tracing::debug!("res: {:?}", m_id);
        Ok((tail, m_id))
}
#[tracing::instrument(name="89720::bit_packed::GameSToonNameDetails::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_region: m_region.expect("Missing m_region from struct"),
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_toon(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSToonNameDetails> {
...
pub fn parse_m_color(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSColor> {
 let (tail, m_color) = GameSColor::parse(input)?;
    tracing::debug!("res: {:?}", m_color);
        Ok((tail, m_color))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_control(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTControlId> {
 let (tail, m_control) = GameTControlId::parse(input)?;
    tracing::debug!("res: {:?}", m_control);
        Ok((tail, m_control))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_team_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTTeamId> {
 let (tail, m_team_id) = GameTTeamId::parse(input)?;
    tracing::debug!("res: {:?}", m_team_id);
        Ok((tail, m_team_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_handicap(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTHandicap> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_result(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEResultDetails> {
 let (tail, m_result) = GameEResultDetails::parse(input)?;
    tracing::debug!("res: {:?}", m_result);
        Ok((tail, m_result))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_working_set_slot_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<Uint8>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSPlayerDetails::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_name: Option<CUserName> = None;
...
let (new_tail, parsed_m_working_set_slot_id) = Self::parse_m_working_set_slot_id(tail)?;
tail = new_tail;
...
m_working_set_slot_id: m_working_set_slot_id.expect("Missing m_working_set_slot_id from struct"),
m_hero: m_hero.expect("Missing m_hero from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameCPlayerDetailsArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 5;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameSPlayerDetails::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
impl GameCModPaths {
#[tracing::instrument(name="89720::GameCModPaths::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
impl GameSDetails {
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_player_list(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameCPlayerDetailsArray>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_disable_recover_game(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mini_save(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_mini_save) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_mini_save);
        Ok((tail, m_mini_save))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_speed(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEGameSpeed> {
 let (tail, m_game_speed) = GameEGameSpeed::parse(input)?;
    tracing::debug!("res: {:?}", m_game_speed);
        Ok((tail, m_game_speed))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_default_difficulty(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTDifficulty> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mod_paths(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<GameCModPaths>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(name="89720::bit_packed::GameSDetails::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_restart_as_transition_map) = Self::parse_m_restart_as_transition_map(tail)?;
tail = new_tail;
m_restart_as_transition_map = Some(parsed_m_restart_as_transition_map);
}
 if m_disable_recover_game.is_none() {
let (new_tail, parsed_m_disable_recover_game) = Self::parse_m_disable_recover_game(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_default_difficulty) = Self::parse_m_default_difficulty(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_player_list: m_player_list.expect("Missing m_player_list from struct"),
...
m_is_blizzard_map: m_is_blizzard_map.expect("Missing m_is_blizzard_map from struct"),
m_time_utc: m_time_utc.expect("Missing m_time_utc from struct"),
m_time_local_offset: m_time_local_offset.expect("Missing m_time_local_offset from struct"),
m_restart_as_transition_map: m_restart_as_transition_map.expect("Missing m_restart_as_transition_map from struct"),
m_disable_recover_game: m_disable_recover_game.expect("Missing m_disable_recover_game from struct"),
m_description: m_description.expect("Missing m_description from struct"),
m_image_file_path: m_image_file_path.expect("Missing m_image_file_path from struct"),
m_campaign_index: m_campaign_index.expect("Missing m_campaign_index from struct"),
...
m_default_difficulty: m_default_difficulty.expect("Missing m_default_difficulty from struct"),
...
impl GameEOptionFog {
#[tracing::instrument(name="89720::GameEOptionFog::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEOptionObservers::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEOptionUserDifficulty::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 3
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEGameLaunch::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 6
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEClientDebugFlags::IntType::Parse::PowExpr", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 64;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_lock_teams(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_lock_teams) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_lock_teams);
        Ok((tail, m_lock_teams))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_teams_together(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_teams_together) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_teams_together);
        Ok((tail, m_teams_together))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_advanced_shared_control(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_advanced_shared_control) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_advanced_shared_control);
        Ok((tail, m_advanced_shared_control))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_random_races(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_random_races) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_random_races);
        Ok((tail, m_random_races))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_battle_net(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_battle_net) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_battle_net);
        Ok((tail, m_battle_net))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_amm(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_amm) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_amm);
        Ok((tail, m_amm))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_competitive(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_competitive) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_competitive);
        Ok((tail, m_competitive))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_practice(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_practice) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_practice);
        Ok((tail, m_practice))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_cooperative(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_cooperative) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_cooperative);
        Ok((tail, m_cooperative))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_no_victory_or_defeat(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_no_victory_or_defeat) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_no_victory_or_defeat);
        Ok((tail, m_no_victory_or_defeat))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_hero_duplicates_allowed(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_hero_duplicates_allowed) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_hero_duplicates_allowed);
        Ok((tail, m_hero_duplicates_allowed))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_fog(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEOptionFog> {
 let (tail, m_fog) = GameEOptionFog::parse(input)?;
    tracing::debug!("res: {:?}", m_fog);
        Ok((tail, m_fog))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_observers(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEOptionObservers> {
 let (tail, m_observers) = GameEOptionObservers::parse(input)?;
    tracing::debug!("res: {:?}", m_observers);
        Ok((tail, m_observers))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_user_difficulty(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEOptionUserDifficulty> {
 let (tail, m_user_difficulty) = GameEOptionUserDifficulty::parse(input)?;
    tracing::debug!("res: {:?}", m_user_difficulty);
        Ok((tail, m_user_difficulty))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_client_debug_flags(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEClientDebugFlags> {
 let (tail, m_client_debug_flags) = GameEClientDebugFlags::parse(input)?;
    tracing::debug!("res: {:?}", m_client_debug_flags);
        Ok((tail, m_client_debug_flags))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_build_coach_enabled(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_build_coach_enabled) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_build_coach_enabled);
        Ok((tail, m_build_coach_enabled))
}
#[tracing::instrument(name="89720::bit_packed::GameSGameOptions::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_advanced_shared_control) = Self::parse_m_advanced_shared_control(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_no_victory_or_defeat) = Self::parse_m_no_victory_or_defeat(tail)?;
tail = new_tail;
m_no_victory_or_defeat = Some(parsed_m_no_victory_or_defeat);
}
 if m_hero_duplicates_allowed.is_none() {
let (new_tail, parsed_m_hero_duplicates_allowed) = Self::parse_m_hero_duplicates_allowed(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_client_debug_flags) = Self::parse_m_client_debug_flags(tail)?;
tail = new_tail;
m_client_debug_flags = Some(parsed_m_client_debug_flags);
}
 if m_build_coach_enabled.is_none() {
let (new_tail, parsed_m_build_coach_enabled) = Self::parse_m_build_coach_enabled(tail)?;
tail = new_tail;
m_build_coach_enabled = Some(parsed_m_build_coach_enabled);
}
Ok((tail, Self {m_lock_teams: m_lock_teams.expect("Missing m_lock_teams from struct"),
m_teams_together: m_teams_together.expect("Missing m_teams_together from struct"),
m_advanced_shared_control: m_advanced_shared_control.expect("Missing m_advanced_shared_control from struct"),
m_random_races: m_random_races.expect("Missing m_random_races from struct"),
m_battle_net: m_battle_net.expect("Missing m_battle_net from struct"),
m_amm: m_amm.expect("Missing m_amm from struct"),
m_competitive: m_competitive.expect("Missing m_competitive from struct"),
m_practice: m_practice.expect("Missing m_practice from struct"),
m_cooperative: m_cooperative.expect("Missing m_cooperative from struct"),
m_no_victory_or_defeat: m_no_victory_or_defeat.expect("Missing m_no_victory_or_defeat from struct"),
m_hero_duplicates_allowed: m_hero_duplicates_allowed.expect("Missing m_hero_duplicates_allowed from struct"),
m_fog: m_fog.expect("Missing m_fog from struct"),
m_observers: m_observers.expect("Missing m_observers from struct"),
m_user_difficulty: m_user_difficulty.expect("Missing m_user_difficulty from struct"),
m_client_debug_flags: m_client_debug_flags.expect("Missing m_client_debug_flags from struct"),
m_build_coach_enabled: m_build_coach_enabled.expect("Missing m_build_coach_enabled from struct"),
}))
}}
...
impl GameEGameType {
#[tracing::instrument(name="89720::GameEGameType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 7
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
impl GameEControl {
#[tracing::instrument(name="89720::GameEControl::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
pub struct GameTControlId {
    pub value: i64,}
impl GameTControlId {
#[tracing::instrument(name="89720::GameTControlId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTControlCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameCAllowedControls::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let bitarray_length_bits: usize = 8;
         let (tail, bitarray_length) = take_n_bits_into_i64(input, bitarray_length_bits)?;
         let (tail, value) = take_bit_array(tail, bitarray_length as usize)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_colors(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCAllowedColors> {
 let (tail, m_allowed_colors) = GameCAllowedColors::parse(input)?;
    tracing::debug!("res: {:?}", m_allowed_colors);
        Ok((tail, m_allowed_colors))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_races(input: (&[u8], usize)) -> IResult<(&[u8], usize), CAllowedRaces> {
 let (tail, m_allowed_races) = CAllowedRaces::parse(input)?;
    tracing::debug!("res: {:?}", m_allowed_races);
        Ok((tail, m_allowed_races))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_difficulty(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCAllowedDifficulty> {
 let (tail, m_allowed_difficulty) = GameCAllowedDifficulty::parse(input)?;
    tracing::debug!("res: {:?}", m_allowed_difficulty);
        Ok((tail, m_allowed_difficulty))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_controls(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCAllowedControls> {
 let (tail, m_allowed_controls) = GameCAllowedControls::parse(input)?;
    tracing::debug!("res: {:?}", m_allowed_controls);
        Ok((tail, m_allowed_controls))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_observe_types(input: (&[u8], usize)) -> IResult<(&[u8], usize), CAllowedObserveTypes> {
 let (tail, m_allowed_observe_types) = CAllowedObserveTypes::parse(input)?;
    tracing::debug!("res: {:?}", m_allowed_observe_types);
        Ok((tail, m_allowed_observe_types))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_allowed_ai_builds(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCAllowedAiBuild> {
...
#[tracing::instrument(name="89720::bit_packed::GameSSlotDescription::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_allowed_difficulty) = Self::parse_m_allowed_difficulty(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_allowed_observe_types) = Self::parse_m_allowed_observe_types(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_allowed_colors: m_allowed_colors.expect("Missing m_allowed_colors from struct"),
m_allowed_races: m_allowed_races.expect("Missing m_allowed_races from struct"),
m_allowed_difficulty: m_allowed_difficulty.expect("Missing m_allowed_difficulty from struct"),
m_allowed_controls: m_allowed_controls.expect("Missing m_allowed_controls from struct"),
m_allowed_observe_types: m_allowed_observe_types.expect("Missing m_allowed_observe_types from struct"),
m_allowed_ai_builds: m_allowed_ai_builds.expect("Missing m_allowed_ai_builds from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameCCacheHandle::BlobType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, _) = byte_align(input)?;
         let num_bits: usize = 6;
         let (tail, value) = take_bit_array(tail, num_bits)?;
...
#[tracing::instrument(name="89720::GameCCacheHandles::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 6;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameCCacheHandle::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(name="89720::GameCGameCacheName::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 11;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameCAuthorName::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 8;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameSSlotDescriptions::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 5;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameSSlotDescription::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_random_value(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_random_value) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_random_value);
        Ok((tail, m_random_value))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_cache_name(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCGameCacheName> {
 let (tail, m_game_cache_name) = GameCGameCacheName::parse(input)?;
    tracing::debug!("res: {:?}", m_game_cache_name);
        Ok((tail, m_game_cache_name))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_options(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSGameOptions> {
 let (tail, m_game_options) = GameSGameOptions::parse(input)?;
    tracing::debug!("res: {:?}", m_game_options);
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEGameType> {
 let (tail, m_game_type) = GameEGameType::parse(input)?;
    tracing::debug!("res: {:?}", m_game_type);
        Ok((tail, m_game_type))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_users(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserCount> {
 let (tail, m_max_users) = TUserCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_users);
        Ok((tail, m_max_users))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_observers(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserCount> {
 let (tail, m_max_observers) = TUserCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_observers);
        Ok((tail, m_max_observers))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_players(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTPlayerCount> {
 let (tail, m_max_players) = GameTPlayerCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_players);
        Ok((tail, m_max_players))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_teams(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTTeamCount> {
 let (tail, m_max_teams) = GameTTeamCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_teams);
        Ok((tail, m_max_teams))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_colors(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTColorCount> {
 let (tail, m_max_colors) = GameTColorCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_colors);
        Ok((tail, m_max_colors))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_races(input: (&[u8], usize)) -> IResult<(&[u8], usize), TRaceCount> {
 let (tail, m_max_races) = TRaceCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_races);
        Ok((tail, m_max_races))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_controls(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTControlCount> {
 let (tail, m_max_controls) = GameTControlCount::parse(input)?;
    tracing::debug!("res: {:?}", m_max_controls);
        Ok((tail, m_max_controls))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_map_size_x(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_map_size_x) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_map_size_x);
        Ok((tail, m_map_size_x))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_map_size_y(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint8> {
 let (tail, m_map_size_y) = Uint8::parse(input)?;
    tracing::debug!("res: {:?}", m_map_size_y);
        Ok((tail, m_map_size_y))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_map_file_sync_checksum(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSyncChecksum> {
 let (tail, m_map_file_sync_checksum) = GameTSyncChecksum::parse(input)?;
    tracing::debug!("res: {:?}", m_map_file_sync_checksum);
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_map_author_name(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCAuthorName> {
 let (tail, m_map_author_name) = GameCAuthorName::parse(input)?;
    tracing::debug!("res: {:?}", m_map_author_name);
        Ok((tail, m_map_author_name))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_mod_file_sync_checksum(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSyncChecksum> {
 let (tail, m_mod_file_sync_checksum) = GameTSyncChecksum::parse(input)?;
    tracing::debug!("res: {:?}", m_mod_file_sync_checksum);
        Ok((tail, m_mod_file_sync_checksum))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_slot_descriptions(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSlotDescriptions> {
 let (tail, m_slot_descriptions) = GameSSlotDescriptions::parse(input)?;
    tracing::debug!("res: {:?}", m_slot_descriptions);
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_default_ai_build(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTaiBuild> {
 let (tail, m_default_ai_build) = GameTaiBuild::parse(input)?;
    tracing::debug!("res: {:?}", m_default_ai_build);
        Ok((tail, m_default_ai_build))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_cache_handles(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCCacheHandles> {
 let (tail, m_cache_handles) = GameCCacheHandles::parse(input)?;
    tracing::debug!("res: {:?}", m_cache_handles);
        Ok((tail, m_cache_handles))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_has_extension_mod(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_has_extension_mod) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_has_extension_mod);
        Ok((tail, m_has_extension_mod))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_has_non_blizzard_extension_mod(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_has_non_blizzard_extension_mod) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_has_non_blizzard_extension_mod);
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_is_premade_ffa(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_is_premade_ffa) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_is_premade_ffa);
        Ok((tail, m_is_premade_ffa))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_is_coop_mode(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_is_coop_mode) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_is_coop_mode);
        Ok((tail, m_is_coop_mode))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_is_realtime_mode(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_is_realtime_mode) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_is_realtime_mode);
        Ok((tail, m_is_realtime_mode))
}
#[tracing::instrument(name="89720::bit_packed::GameSGameDescription::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
let (new_tail, parsed_m_map_file_sync_checksum) = Self::parse_m_map_file_sync_checksum(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_mod_file_sync_checksum) = Self::parse_m_mod_file_sync_checksum(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_has_non_blizzard_extension_mod) = Self::parse_m_has_non_blizzard_extension_mod(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_random_value: m_random_value.expect("Missing m_random_value from struct"),
m_game_cache_name: m_game_cache_name.expect("Missing m_game_cache_name from struct"),
m_game_options: m_game_options.expect("Missing m_game_options from struct"),
m_game_speed: m_game_speed.expect("Missing m_game_speed from struct"),
m_game_type: m_game_type.expect("Missing m_game_type from struct"),
m_max_users: m_max_users.expect("Missing m_max_users from struct"),
m_max_observers: m_max_observers.expect("Missing m_max_observers from struct"),
m_max_players: m_max_players.expect("Missing m_max_players from struct"),
m_max_teams: m_max_teams.expect("Missing m_max_teams from struct"),
m_max_colors: m_max_colors.expect("Missing m_max_colors from struct"),
m_max_races: m_max_races.expect("Missing m_max_races from struct"),
m_max_controls: m_max_controls.expect("Missing m_max_controls from struct"),
m_map_size_x: m_map_size_x.expect("Missing m_map_size_x from struct"),
m_map_size_y: m_map_size_y.expect("Missing m_map_size_y from struct"),
m_map_file_sync_checksum: m_map_file_sync_checksum.expect("Missing m_map_file_sync_checksum from struct"),
m_map_file_name: m_map_file_name.expect("Missing m_map_file_name from struct"),
m_map_author_name: m_map_author_name.expect("Missing m_map_author_name from struct"),
m_mod_file_sync_checksum: m_mod_file_sync_checksum.expect("Missing m_mod_file_sync_checksum from struct"),
m_slot_descriptions: m_slot_descriptions.expect("Missing m_slot_descriptions from struct"),
m_default_difficulty: m_default_difficulty.expect("Missing m_default_difficulty from struct"),
m_default_ai_build: m_default_ai_build.expect("Missing m_default_ai_build from struct"),
m_cache_handles: m_cache_handles.expect("Missing m_cache_handles from struct"),
m_has_extension_mod: m_has_extension_mod.expect("Missing m_has_extension_mod from struct"),
m_has_non_blizzard_extension_mod: m_has_non_blizzard_extension_mod.expect("Missing m_has_non_blizzard_extension_mod from struct"),
m_is_blizzard_map: m_is_blizzard_map.expect("Missing m_is_blizzard_map from struct"),
m_is_premade_ffa: m_is_premade_ffa.expect("Missing m_is_premade_ffa from struct"),
m_is_coop_mode: m_is_coop_mode.expect("Missing m_is_coop_mode from struct"),
m_is_realtime_mode: m_is_realtime_mode.expect("Missing m_is_realtime_mode from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameTLobbySlotCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 5;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTLobbySlotId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameCArtifactArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 4;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(CArtifactHandle::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(name="89720::GameCCommanderMasteryTalentArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameCRetryMutationIndexArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 3;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(Uint32::parse, array_length as usize)(tail)?;
...
pub struct GameTReward {
    pub value: Uint32,}
impl GameTReward {
#[tracing::instrument(name="89720::GameTReward::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameCRewardArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_key(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_key) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_key);
        Ok((tail, m_key))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_rewards(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCRewardArray> {
 let (tail, m_rewards) = GameCRewardArray::parse(input)?;
    tracing::debug!("res: {:?}", m_rewards);
        Ok((tail, m_rewards))
}
#[tracing::instrument(name="89720::bit_packed::GameCRewardOverride::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
#[tracing::instrument(name="89720::GameCRewardOverrideArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 17;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameCRewardOverride::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
pub struct GameTLicense {
    pub value: Uint32,}
impl GameTLicense {
#[tracing::instrument(name="89720::GameTLicense::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(name="89720::GameCLicenseArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 16;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameTLicense::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(name="89720::GameTFlexLicenseName::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 8;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameTFlexLicenseAttributeName::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 8;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(name="89720::GameTFlexLicenseAttributeValue::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 11;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_color_pref(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTColorPreference> {
 let (tail, m_color_pref) = GameTColorPreference::parse(input)?;
    tracing::debug!("res: {:?}", m_color_pref);
        Ok((tail, m_color_pref))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_race_pref(input: (&[u8], usize)) -> IResult<(&[u8], usize), TRacePreference> {
 let (tail, m_race_pref) = TRacePreference::parse(input)?;
    tracing::debug!("res: {:?}", m_race_pref);
        Ok((tail, m_race_pref))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_difficulty(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTDifficulty> {
 let (tail, m_difficulty) = GameTDifficulty::parse(input)?;
    tracing::debug!("res: {:?}", m_difficulty);
        Ok((tail, m_difficulty))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_ai_build(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTaiBuild> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_logo_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTPlayerLogoIndex> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_artifacts(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCArtifactArray> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_licenses(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCLicenseArray> {
 let (tail, m_licenses) = GameCLicenseArray::parse(input)?;
    tracing::debug!("res: {:?}", m_licenses);
        Ok((tail, m_licenses))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_tandem_leader_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<TUserId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_commander(input: (&[u8], usize)) -> IResult<(&[u8], usize), CCommanderHandle> {
 let (tail, m_commander) = CCommanderHandle::parse(input)?;
    tracing::debug!("res: {:?}", m_commander);
        Ok((tail, m_commander))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_commander_level(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_tandem_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<TUserId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_commander_mastery_level(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_commander_mastery_talents(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCCommanderMasteryTalentArray> {
 let (tail, m_commander_mastery_talents) = GameCCommanderMasteryTalentArray::parse(input)?;
    tracing::debug!("res: {:?}", m_commander_mastery_talents);
        Ok((tail, m_commander_mastery_talents))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_trophy_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_trophy_id) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_trophy_id);
        Ok((tail, m_trophy_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_reward_overrides(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCRewardOverrideArray> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_brutal_plus_difficulty(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_brutal_plus_difficulty) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_brutal_plus_difficulty);
        Ok((tail, m_brutal_plus_difficulty))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_retry_mutation_indexes(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCRetryMutationIndexArray> {
 let (tail, m_retry_mutation_indexes) = GameCRetryMutationIndexArray::parse(input)?;
    tracing::debug!("res: {:?}", m_retry_mutation_indexes);
        Ok((tail, m_retry_mutation_indexes))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_a_c_enemy_race(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_a_c_enemy_race) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_a_c_enemy_race);
        Ok((tail, m_a_c_enemy_race))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_a_c_enemy_wave_type(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
 let (tail, m_a_c_enemy_wave_type) = Uint32::parse(input)?;
    tracing::debug!("res: {:?}", m_a_c_enemy_wave_type);
        Ok((tail, m_a_c_enemy_wave_type))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_selected_commander_prestige(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
#[tracing::instrument(name="89720::bit_packed::GameSLobbySlot::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_control: Option<GameTControlId> = None;
...
let (new_tail, parsed_m_has_silence_penalty) = Self::parse_m_has_silence_penalty(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_commander_mastery_level) = Self::parse_m_commander_mastery_level(tail)?;
tail = new_tail;
m_commander_mastery_level = Some(parsed_m_commander_mastery_level);
}
 if m_commander_mastery_talents.is_none() {
let (new_tail, parsed_m_commander_mastery_talents) = Self::parse_m_commander_mastery_talents(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_brutal_plus_difficulty) = Self::parse_m_brutal_plus_difficulty(tail)?;
tail = new_tail;
m_brutal_plus_difficulty = Some(parsed_m_brutal_plus_difficulty);
}
 if m_retry_mutation_indexes.is_none() {
let (new_tail, parsed_m_retry_mutation_indexes) = Self::parse_m_retry_mutation_indexes(tail)?;
tail = new_tail;
...
let (new_tail, parsed_m_a_c_enemy_wave_type) = Self::parse_m_a_c_enemy_wave_type(tail)?;
tail = new_tail;
m_a_c_enemy_wave_type = Some(parsed_m_a_c_enemy_wave_type);
}
 if m_selected_commander_prestige.is_none() {
let (new_tail, parsed_m_selected_commander_prestige) = Self::parse_m_selected_commander_prestige(tail)?;
tail = new_tail;
m_selected_commander_prestige = Some(parsed_m_selected_commander_prestige);
}
Ok((tail, Self {m_control: m_control.expect("Missing m_control from struct"),
m_user_id: m_user_id.expect("Missing m_user_id from struct"),
m_team_id: m_team_id.expect("Missing m_team_id from struct"),
...
m_tandem_leader_id: m_tandem_leader_id.expect("Missing m_tandem_leader_id from struct"),
m_commander: m_commander.expect("Missing m_commander from struct"),
m_commander_level: m_commander_level.expect("Missing m_commander_level from struct"),
m_has_silence_penalty: m_has_silence_penalty.expect("Missing m_has_silence_penalty from struct"),
m_tandem_id: m_tandem_id.expect("Missing m_tandem_id from struct"),
m_commander_mastery_level: m_commander_mastery_level.expect("Missing m_commander_mastery_level from struct"),
m_commander_mastery_talents: m_commander_mastery_talents.expect("Missing m_commander_mastery_talents from struct"),
m_trophy_id: m_trophy_id.expect("Missing m_trophy_id from struct"),
m_reward_overrides: m_reward_overrides.expect("Missing m_reward_overrides from struct"),
m_brutal_plus_difficulty: m_brutal_plus_difficulty.expect("Missing m_brutal_plus_difficulty from struct"),
m_retry_mutation_indexes: m_retry_mutation_indexes.expect("Missing m_retry_mutation_indexes from struct"),
m_a_c_enemy_race: m_a_c_enemy_race.expect("Missing m_a_c_enemy_race from struct"),
m_a_c_enemy_wave_type: m_a_c_enemy_wave_type.expect("Missing m_a_c_enemy_wave_type from struct"),
m_selected_commander_prestige: m_selected_commander_prestige.expect("Missing m_selected_commander_prestige from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameCLobbySlotArray::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 5;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameSLobbySlot::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(name="89720::GameSLobbySlotChange::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// ChoiceType:
            // Use the number of elements in the json .fields to calculate how many
            // bits to have unique tags.
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_phase(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEPhase> {
 let (tail, m_phase) = GameEPhase::parse(input)?;
    tracing::debug!("res: {:?}", m_phase);
        Ok((tail, m_phase))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_max_users(input: (&[u8], usize)) -> IResult<(&[u8], usize), TUserCount> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_slots(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCLobbySlotArray> {
 let (tail, m_slots) = GameCLobbySlotArray::parse(input)?;
    tracing::debug!("res: {:?}", m_slots);
        Ok((tail, m_slots))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_random_seed(input: (&[u8], usize)) -> IResult<(&[u8], usize), Uint32> {
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_host_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), Option<TUserId>> {
let (tail, is_provided): ((&[u8], usize), bool) = parse_bool(input)?;
...
let (tail, res) = TUserId::parse(tail)?;
    ...
    tracing::debug!("res: {:?}", m_host_user_id);
Ok((tail, m_host_user_id))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_is_single_player(input: (&[u8], usize)) -> IResult<(&[u8], usize), bool> {
 let (tail, m_is_single_player) = parse_bool(input)?;
    tracing::debug!("res: {:?}", m_is_single_player);
...
#[tracing::instrument(name="89720::bit_packed::GameSLobbyState::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_phase: m_phase.expect("Missing m_phase from struct"),
m_max_users: m_max_users.expect("Missing m_max_users from struct"),
m_max_observers: m_max_observers.expect("Missing m_max_observers from struct"),
m_slots: m_slots.expect("Missing m_slots from struct"),
m_random_seed: m_random_seed.expect("Missing m_random_seed from struct"),
m_host_user_id: m_host_user_id.expect("Missing m_host_user_id from struct"),
m_is_single_player: m_is_single_player.expect("Missing m_is_single_player from struct"),
m_picked_map_tag: m_picked_map_tag.expect("Missing m_picked_map_tag from struct"),
m_game_duration: m_game_duration.expect("Missing m_game_duration from struct"),
m_default_difficulty: m_default_difficulty.expect("Missing m_default_difficulty from struct"),
m_default_ai_build: m_default_ai_build.expect("Missing m_default_ai_build from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_user_initial_data(input: (&[u8], usize)) -> IResult<(&[u8], usize), CUserInitialDataArray> {
 let (tail, m_user_initial_data) = CUserInitialDataArray::parse(input)?;
    tracing::debug!("res: {:?}", m_user_initial_data);
        Ok((tail, m_user_initial_data))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_game_description(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSGameDescription> {
 let (tail, m_game_description) = GameSGameDescription::parse(input)?;
    tracing::debug!("res: {:?}", m_game_description);
        Ok((tail, m_game_description))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_lobby_state(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSLobbyState> {
 let (tail, m_lobby_state) = GameSLobbyState::parse(input)?;
    tracing::debug!("res: {:?}", m_lobby_state);
        Ok((tail, m_lobby_state))
}
#[tracing::instrument(name="89720::bit_packed::GameSLobbySyncState::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_user_initial_data: m_user_initial_data.expect("Missing m_user_initial_data from struct"),
m_game_description: m_game_description.expect("Missing m_game_description from struct"),
m_lobby_state: m_lobby_state.expect("Missing m_lobby_state from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameEMessageRecipient::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 5
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameCChatString::StringType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let str_size_num_bits: usize = 11;
let (tail, str_size) = parse_packed_int(input, 0, str_size_num_bits)?;
let (tail, _) = byte_align(tail)?;
...
impl GameEMessageId {
#[tracing::instrument(name="89720::GameEMessageId::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 5
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_recipient(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameEMessageRecipient> {
 let (tail, m_recipient) = GameEMessageRecipient::parse(input)?;
    tracing::debug!("res: {:?}", m_recipient);
        Ok((tail, m_recipient))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_string(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameCChatString> {
 let (tail, m_string) = GameCChatString::parse(input)?;
    tracing::debug!("res: {:?}", m_string);
        Ok((tail, m_string))
}
#[tracing::instrument(name="89720::bit_packed::GameSChatMessage::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_recipient: m_recipient.expect("Missing m_recipient from struct"),
m_string: m_string.expect("Missing m_string from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSPingMessage::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_recipient: Option<GameEMessageRecipient> = None;
let mut m_point: Option<GameSPoint> = None;
...
Ok((tail, Self {m_recipient: m_recipient.expect("Missing m_recipient from struct"),
m_point: m_point.expect("Missing m_point from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_progress(input: (&[u8], usize)) -> IResult<(&[u8], usize), Int32> {
 let (tail, m_progress) = Int32::parse(input)?;
    tracing::debug!("res: {:?}", m_progress);
        Ok((tail, m_progress))
}
#[tracing::instrument(name="89720::bit_packed::GameSLoadingProgressMessage::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_progress: m_progress.expect("Missing m_progress from struct"),
}))
}}
...
#[tracing::instrument(name="89720::bit_packed::GameSServerPingMessage::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let tail = input;
         Ok((tail, Self {}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_status(input: (&[u8], usize)) -> IResult<(&[u8], usize), EReconnectStatus> {
 let (tail, m_status) = EReconnectStatus::parse(input)?;
    tracing::debug!("res: {:?}", m_status);
        Ok((tail, m_status))
}
#[tracing::instrument(name="89720::bit_packed::GameSReconnectNotifyMessage::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_status: m_status.expect("Missing m_status from struct"),
}))
}}
...
pub struct GameTPlayerId {
    pub value: i64,}
impl GameTPlayerId {
#[tracing::instrument(name="89720::GameTPlayerId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTPlayerCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 5;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameEResultCode::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 4
let num_bits: usize = 2;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameEControlGroupUpdate::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// Total fields: 6
let num_bits: usize = 3;
let (tail, variant_tag) = parse_packed_int(input, 0, num_bits)?;
...
#[tracing::instrument(name="89720::GameTSelectionCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
...
#[tracing::instrument(name="89720::GameTSelectionIndex::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 9;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTSubgroupPriority::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 8;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTSubgroupCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 9;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTSubgroupIndex::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 9;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTControlGroupCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTControlGroupIndex::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(name="89720::GameTControlGroupId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_subgroup_priority(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSubgroupPriority> {
 let (tail, m_subgroup_priority) = GameTSubgroupPriority::parse(input)?;
    tracing::debug!("res: {:?}", m_subgroup_priority);
        Ok((tail, m_subgroup_priority))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_intra_subgroup_priority(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSubgroupPriority> {
 let (tail, m_intra_subgroup_priority) = GameTSubgroupPriority::parse(input)?;
    tracing::debug!("res: {:?}", m_intra_subgroup_priority);
        Ok((tail, m_intra_subgroup_priority))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_count(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSelectionCount> {
 let (tail, m_count) = GameTSelectionCount::parse(input)?;
    tracing::debug!("res: {:?}", m_count);
        Ok((tail, m_count))
}
#[tracing::instrument(name="89720::bit_packed::GameSSelectionDeltaSubgroup::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_unit_link: Option<GameTUnitLink> = None;
...
let (new_tail, parsed_m_intra_subgroup_priority) = Self::parse_m_intra_subgroup_priority(tail)?;
tail = new_tail;
...
Ok((tail, Self {m_unit_link: m_unit_link.expect("Missing m_unit_link from struct"),
m_subgroup_priority: m_subgroup_priority.expect("Missing m_subgroup_priority from struct"),
m_intra_subgroup_priority: m_intra_subgroup_priority.expect("Missing m_intra_subgroup_priority from struct"),
m_count: m_count.expect("Missing m_count from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameSelectionIndexArrayType::ArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let array_length_num_bits: usize = 9;
let (tail, array_length) = parse_packed_int(input, 0, array_length_num_bits)?;
let (tail, value) = nom::multi::count(GameTSelectionIndex::parse, array_length as usize)(tail)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
}}
...
#[tracing::instrument(name="89720::GameSelectionMaskType::BitArrayType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
...
#[tracing::instrument(name="89720::GameSSelectionMask::ChoiceType::parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
// ChoiceType:
            // Use the number of elements in the json .fields to calculate how many
            // bits to have unique tags.
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_subgroup_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSubgroupIndex> {
 let (tail, m_subgroup_index) = GameTSubgroupIndex::parse(input)?;
    tracing::debug!("res: {:?}", m_subgroup_index);
        Ok((tail, m_subgroup_index))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_remove_mask(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameSSelectionMask> {
 let (tail, m_remove_mask) = GameSSelectionMask::parse(input)?;
    tracing::debug!("res: {:?}", m_remove_mask);
        Ok((tail, m_remove_mask))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_add_subgroups(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<GameSSelectionDeltaSubgroup>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 9)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
pub fn parse_m_add_unit_tags(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<GameTUnitTag>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 9)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSSelectionDelta::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_subgroup_index: m_subgroup_index.expect("Missing m_subgroup_index from struct"),
m_remove_mask: m_remove_mask.expect("Missing m_remove_mask from struct"),
m_add_subgroups: m_add_subgroups.expect("Missing m_add_subgroups from struct"),
m_add_unit_tags: m_add_unit_tags.expect("Missing m_add_unit_tags from struct"),
}))
}}
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_subgroup_count(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSubgroupCount> {
 let (tail, m_subgroup_count) = GameTSubgroupCount::parse(input)?;
    tracing::debug!("res: {:?}", m_subgroup_count);
        Ok((tail, m_subgroup_count))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_active_subgroup_index(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSubgroupIndex> {
 let (tail, m_active_subgroup_index) = GameTSubgroupIndex::parse(input)?;
    tracing::debug!("res: {:?}", m_active_subgroup_index);
        Ok((tail, m_active_subgroup_index))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_unit_tags_checksum(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSyncChecksum> {
 let (tail, m_unit_tags_checksum) = GameTSyncChecksum::parse(input)?;
    tracing::debug!("res: {:?}", m_unit_tags_checksum);
        Ok((tail, m_unit_tags_checksum))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_subgroup_indices_checksum(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSyncChecksum> {
 let (tail, m_subgroup_indices_checksum) = GameTSyncChecksum::parse(input)?;
    tracing::debug!("res: {:?}", m_subgroup_indices_checksum);
        Ok((tail, m_subgroup_indices_checksum))
}
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_subgroups_checksum(input: (&[u8], usize)) -> IResult<(&[u8], usize), GameTSyncChecksum> {
...
#[tracing::instrument(name="89720::bit_packed::GameSSelectionSyncData::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_count: Option<GameTSelectionCount> = None;
...
let (new_tail, parsed_m_active_subgroup_index) = Self::parse_m_active_subgroup_index(tail)?;
tail = new_tail;
m_active_subgroup_index = Some(parsed_m_active_subgroup_index);
}
 if m_unit_tags_checksum.is_none() {
let (new_tail, parsed_m_unit_tags_checksum) = Self::parse_m_unit_tags_checksum(tail)?;
tail = new_tail;
m_unit_tags_checksum = Some(parsed_m_unit_tags_checksum);
}
 if m_subgroup_indices_checksum.is_none() {
let (new_tail, parsed_m_subgroup_indices_checksum) = Self::parse_m_subgroup_indices_checksum(tail)?;
tail = new_tail;
m_subgroup_indices_checksum = Some(parsed_m_subgroup_indices_checksum);
}
 if m_subgroups_checksum.is_none() {
let (new_tail, parsed_m_subgroups_checksum) = Self::parse_m_subgroups_checksum(tail)?;
tail = new_tail;
m_subgroups_checksum = Some(parsed_m_subgroups_checksum);
}
Ok((tail, Self {m_count: m_count.expect("Missing m_count from struct"),
m_subgroup_count: m_subgroup_count.expect("Missing m_subgroup_count from struct"),
m_active_subgroup_index: m_active_subgroup_index.expect("Missing m_active_subgroup_index from struct"),
m_unit_tags_checksum: m_unit_tags_checksum.expect("Missing m_unit_tags_checksum from struct"),
m_subgroup_indices_checksum: m_subgroup_indices_checksum.expect("Missing m_subgroup_indices_checksum from struct"),
m_subgroups_checksum: m_subgroups_checksum.expect("Missing m_subgroups_checksum from struct"),
}))
}}
...
#[tracing::instrument(name="89720::GameTSyncChecksum::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint32::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
pub struct GameTSyncValue {
    pub value: Uint16,}
impl GameTSyncValue {
#[tracing::instrument(name="89720::GameTSyncValue::UserType::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let (tail, value) = Uint16::parse(input)?;
// TODO: Unsure about this. 
Ok((tail, Self { value }))
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_checksums(input: (&[u8], usize)) -> IResult<(&[u8], usize), Vec<GameTSyncChecksum>> {
let (mut tail, array_length) = take_n_bits_into_i64(input, 6)?;
let array_length = array_length as usize;
tracing::debug!("Reading array length: {array_length}");
 let mut array = vec![];
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSSessionSyncInfo::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_checksums: m_checksums.expect("Missing m_checksums from struct"),
}))
}}
...
Ok((tail, array))
}#[tracing::instrument(name="89720::bit_packed::GameSGameSyncInfo::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
         let mut m_checksums: Option<Vec<GameTSyncChecksum>> = None;
 if m_checksums.is_none() {
...
pub struct GameTTeamId {
    pub value: i64,}
impl GameTTeamId {
#[tracing::instrument(name="89720::GameTTeamId::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 0;
         let num_bits: usize = 4;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
pub struct GameTTeamCount {
    pub value: i64,}
impl GameTTeamCount {
#[tracing::instrument(name="89720::GameTTeamCount::IntType::Parse::MinMaxConstraint", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let offset: i64 = 1;
         let num_bits: usize = 5;
         let (tail, res) = parse_packed_int(input, offset, num_bits)?;
...
#[tracing::instrument(level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse_m_user_id(input: (&[u8], usize)) -> IResult<(&[u8], usize), i64> {
 let (tail, m_user_id) = parse_packed_int(input, 0, 5usize)?;
    tracing::debug!("res: {:?}", m_user_id);
        Ok((tail, m_user_id))
}
#[tracing::instrument(name="89720::bit_packed::ReplaySGameUserId::Parse", level = "debug", skip(input), fields(peek = peek_bits(input)))]
pub fn parse(input: (&[u8], usize)) -> IResult<(&[u8], usize), Self> {
let mut tail = input;
...
Ok((tail, Self {m_user_id: m_user_id.expect("Missing m_user_id from struct"),
}))
}}
